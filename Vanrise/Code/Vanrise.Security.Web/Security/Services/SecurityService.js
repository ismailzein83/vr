...
app.service('SecurityService', ['$rootScope', 'Sec_CookieService', 'UtilsService', 'VR_Sec_PermissionFlagEnum', '$cookies', 'VR_Sec_SecurityAPIService', '$q', 'VRNotificationService', 'VRModalService',
    function ($rootScope, Sec_CookieService, UtilsService, VR_Sec_PermissionFlagEnum, $cookies, VR_Sec_SecurityAPIService, $q, VRNotificationService, VRModalService) {
        ...
        function redirectToRegisteredApplication(applicationURL, applicationId) {
            var deferred = $q.defer();

            VR_Sec_SecurityAPIService.RedirectToRegisteredApplication(applicationId).then(function (response) {
                if (response != undefined) {
                    Sec_CookieService.createAccessCookieFromAuthToken(response.AuthenticationToken, response.CookieName);
                }
                deferred.resolve();
        ...
        function redirectToCentralApplication() {
            ...
            VR_Sec_SecurityAPIService.RedirectToCentralApplication().then(function (response) {
                if (response != undefined && response.ApplicationRedirectOutput != undefined) {
                    Sec_CookieService.createAccessCookieFromAuthToken(response.ApplicationRedirectOutput.AuthenticationToken, response.ApplicationRedirectOutput.CookieName);
                }
                deferred.resolve();

                if (response != undefined)
                    window.location.href = response.CentralApplicationURL;
