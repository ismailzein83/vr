...
app.directive('vrValidator', ['$compile', 'UtilsService', function ($compile, UtilsService) {
    return {
        ...
        compile: function (tElement, tAttrs) {
            ...
            var newElement = '<div style="min-height: 28px;display: display: inline;" validator-section  ng-class="{\'required-inpute\' : ' + tAttrs.validate + ' != null }" ng-mouseenter="' + showTooltipVariableName + '=true;$root.onValidationMessageShown($event)" ng-mouseleave="' + showTooltipVariableName + '=false" >'
                + '<div style="min-height: 28px; display: inline;" >' + tElement.context.innerHTML + '</div>'
            + validationMessage
            + '</div>';
