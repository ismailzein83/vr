        ...
        template: function (attrs) {
            ...
            var htmlTempalte = '<div class="panel-primary panel-vr ' + focusClass + '"  ng-class="classlevel" >'
				    + '<div class="panel-heading" ng-init="expandname=true" expanded="{{expandname}}" id="{{sectionId}}" ng-click="$root.addFocusPanelClass($event)">'
                        ...
                        + '<label style="padding:0px 2px;" ng-if="!sectionCtrl.headerEditable">{{ sectionCtrl.header }}</label>'
                        + '<span style="padding:0px 2px;" ng-if="sectionCtrl.headerEditable"><span title="{{sectionCtrl.header}}" class="edit-header-container single-line {{dragable || sectionCtrl.enablesortable ? \'share-space\' : \'\'}} {{ !sectionCtrl.header ? \'required-header\' : \'\' }}"   contenteditable="true" ng-model="sectionCtrl.header"></span></span>'
