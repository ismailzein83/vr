...
app.directive('vrSectionV2', ['UtilsService', 'MultiTranscludeService', function (UtilsService, MultiTranscludeService) {
    ...
    var directiveDefinitionObject = {
        ...
        controller: function ($scope, $element, $attrs) {
            var ctrl = this;
            $scope.title = $attrs.header || $attrs.title;
            $scope.classlevel = "1";
            $scope.title = $attrs.header || $attrs.title;
        ...
        template: function (attrs) {
            var htmlTempalte = '<div class="panel-primary panel-vr" ng-class="classlevel" >'
                + '<div class="panel-heading" ng-init="expandname=true" expanded="{{expandname}}"><label><span style="padding-right: 3px;" class="hand-cursor glyphicon " ng-show="collapsible" ng-click=" expandname =!expandname " ng-class=" expandname ?\'glyphicon-collapse-up\':\'glyphicon-collapse-down\'" ></span>{{title}}</label><div class="section-menu" ng-if="sectionCtrl.menuactions.length > 0"><vr-button type="SectionAction" menuactions="sectionCtrl.menuactions" isasynchronous="true" standalone></vr-button></div></div>'
