        ...
        template: function (element) {
            var template = '<vr-row removeline>'
                                ...
                                + '    <vr-tab-header-links selectedindex="ctrl.selectedTabIndex" onselectionchanged="ctrl.tabSelectionChanged()">'
                                  + '      <vr-tab-header-link ng-repeat="tab in ctrl.tabs" isselected="tab.isSelected">{{ tab.header }} <span ng-if="tab.validationContext.validate() != null" style="color:red" title="has validation errors!">*</span></vr-tab-header-link>'
                                    + '</vr-tab-header-links>'
                                + '</vr-columns>'
                            + '</vr-row>';
            return template;
