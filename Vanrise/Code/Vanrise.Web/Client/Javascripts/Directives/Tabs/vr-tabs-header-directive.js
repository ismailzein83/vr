    ...
    var directiveDefinitionObject = {
        ...
        template: function (element) {
            ...
            var template = '<vr-tab-header-links ' + verticalflag + ' ' + flatflag + ' ' + hidepaginationcontrols + '  backwardvisible="{{ctrl.isBackwardPaginationVisible()}}" forwardvisible="{{ctrl.isForwardPaginationVisible()}}" selectedindex="ctrl.selectedTabIndex" onselectionchanged="ctrl.tabSelectionChanged()" ng-if="ctrl.tabs.length > 0" >'
                ...
                + ' <span class="vr-header-label {{tab.onremove || ctrl.sortable ?\'with-tab-control\':\'\'  }} {{(tab.onremove || ctrl.sortable) &&  tab.iconurl ?\'icon-space\':\'\'  }}"  title="{{tab.tabItem[ctrl.datatitlefield]}}" ng-if="ctrl.datatitlefield" ng-class="!tab.tabItem[ctrl.datatitlefield] ?\'required-header\':\'\' " contenteditable="true" ng-model="tab.tabItem[ctrl.datatitlefield]"></span> '
