(function (app) {
	...
	function IsEmptyFilterDefintionSettingsDirective(UtilsService, VRUIUtilsService, VRLocalizationService) {
		...
		function IsEmptyFilterDefintionSettingsController($scope, ctrl, $attrs) {
			...
			var fillSelectorDeferredReady = UtilsService.createPromiseDeferred();
			function initializeController() {
				$scope.scopeModel = {};
				$scope.scopeModel.fields = [];
				$scope.scopeModel.showResourceKeyColumn = VRLocalizationService.isLocalizationEnabled();

				$scope.scopeModel.onDataRecordTypeSelectorChanged = function (option) {
					if (option != undefined) {
						if (fillSelectorDeferredReady != undefined) {
							fillSelectorDeferredReady.resolve();
						}
						else {
							$scope.scopeModel.fieldTitle = option.Title;
						}
					}
				};
			...
			function defineAPI() {
				...
				api.load = function (payload) {
					...
					return UtilsService.waitPromiseNode(rootPromiseNode).finally(function () {
						fillSelectorDeferredReady = undefined;
					});
				};
