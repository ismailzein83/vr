'use strict';
app.directive('vrGenericdataDatatransformationdefinitionrecordSelector', ['UtilsService', '$compile', 'VRUIUtilsService', 'VR_GenericData_DataRecordTypeAPIService', function (UtilsService, $compile, VRUIUtilsService, VR_GenericData_DataRecordTypeAPIService) {
    ...
    function getDataTransformationDefinitonTemplate(attrs) {
        ...
        if (attrs.showaddbutton != undefined)
            addCliked = 'onaddclicked="addDataTransformationRecord"';

        return '<div>'
            + '<vr-select ' + multipleselection + '  datatextfield="Name" datavaluefield="Name" isrequired="ctrl.isrequired"'
            + ' label="' + label + '" ' + addCliked + ' datasource="ctrl.datasource" on-ready="ctrl.onSelectorReady" selectedvalues="ctrl.selectedvalues" vr-disabled="ctrl.isdisabled" onselectionchanged="ctrl.onselectionchanged" entityName="Data Transformation Record" onselectitem="ctrl.onselectitem" ondeselectitem="ctrl.ondeselectitem"></vr-select>'
            + '</div>'
    }
