...
namespace Vanrise.Common
{
    public static class Utilities
    {
        ...
        public static DateTime? ConvertToExcelDateTime(DateTime? dateToParse)
        {
            if (!dateToParse.HasValue) return null;
            return ConvertToExcelDateTime(dateToParse.Value);
        }

        public static DateTime ConvertToExcelDateTime(DateTime date)
        {
            DateTime convertedDateTime = new DateTime(date.Year, date.Month, date.Day, date.Hour, date.Minute, date.Second, date.Millisecond);
            if (convertedDateTime.Hour == 23 && convertedDateTime.Minute == 59 && convertedDateTime.Second == 59)
            {
                convertedDateTime = new DateTime(date.Year, date.Month, date.Day, date.Hour, date.Minute, date.Second, 000);
        ...
        {
            Tlist = Tlist.OrderBy(item => item.BED).ToList();
            Qlist = Qlist.OrderBy(item => item.BED).ToList();
            int Tindex = 0;
            int Qindex = 0;
            ...
            List<VRDateTimeRange> dateRanges = new List<VRDateTimeRange>();
            VRDateTimeRange lastDateRange = new VRDateTimeRange();

            while (Tindex < TlistCount || Qindex < QlistCount)
            {
                VRDateTimeRange dateRangeToAdd = new VRDateTimeRange();
                Q Qitem = Qindex < QlistCount ? Qlist[Qindex] : null;
                T Titem = Tindex < TlistCount ? Tlist[Tindex] : null;
