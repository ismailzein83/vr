    ...
    public class RORangeBranchRouteSettings : BranchRouteSettings
    {
        ...
        public override List<BranchRoute> GenerateBranchRoutes(IGenerateBranchRoutesContext context)
        {
            ...
            return (result != null || result.Count > 0) ? result.MapRecords((item) =>
            {
                return new BranchRoute()
                {
                    ...
                    OverflowOnFirstOptionOnly = item.OverflowOnFirstOptionOnly
                };
            }).ToList() : null;
        }
        ...
        public List<RORangeBranchRoute> MergeBranchRoutes(List<RORangeBranchRoute> branchRoutes, bool checkIncludeSwitch)
        {
            ...
            for (int i = 1; i < orderedBRs.Count; i++)
            {
                ...
                if (pre.To.HasValue && pre.To == cur.From - 1 && (!checkIncludeSwitch || (pre.IncludeTrunkAsSwitch == cur.IncludeTrunkAsSwitch && pre.OverflowOnFirstOptionOnly == cur.OverflowOnFirstOptionOnly)))
                {
                    pre = new RORangeBranchRoute { From = pre.From, To = cur.To, IncludeTrunkAsSwitch = !checkIncludeSwitch ? false : cur.IncludeTrunkAsSwitch, OverflowOnFirstOptionOnly = !checkIncludeSwitch ? false : cur.OverflowOnFirstOptionOnly };
                }
                else if (!pre.To.HasValue && pre.From == cur.From - 1 && (!checkIncludeSwitch || (pre.IncludeTrunkAsSwitch == cur.IncludeTrunkAsSwitch && pre.OverflowOnFirstOptionOnly == cur.OverflowOnFirstOptionOnly)))
                {
                    pre = new RORangeBranchRoute { From = pre.From, To = cur.To, IncludeTrunkAsSwitch = !checkIncludeSwitch ? false : cur.IncludeTrunkAsSwitch, OverflowOnFirstOptionOnly = !checkIncludeSwitch ? false : cur.OverflowOnFirstOptionOnly };
                }
                else
                {
                    result.Add(pre);
                    pre = new RORangeBranchRoute() { From = cur.From, To = cur.To, IncludeTrunkAsSwitch = cur.IncludeTrunkAsSwitch, OverflowOnFirstOptionOnly = cur.OverflowOnFirstOptionOnly };
                }
            }
            result.Add(pre);
            return result;
