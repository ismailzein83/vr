...
namespace TABS.Components
{
    public class PricingGenerator : IDisposable
    {
        ...
        protected class DateSensitiveEntityCache<T> where T : Interfaces.IZoneSupplied
        {
            ...
            {
                Load(session, pricingStart, IsRepricing);
            }

            protected void Load(NHibernate.ISession session, DateTime pricingStart, bool IsRepricing)
            {
                ...
                IList<T> all = PricingGeneratorEntites<T>.Load(pricingStart, IsRepricing);
                foreach (T entity in all)
                {
                    if (entity.Supplier == null) continue;
                    TABS.CarrierAccount customer = entity.Customer;
                    TABS.CarrierAccount supplier = entity.Supplier;
                    TABS.Zone zone = (entity.Zone == null) ? AnyZone : entity.Zone;

                    // Our Entities
                    if (supplier.Equals(TABS.CarrierAccount.SYSTEM))
                    {
                        Dictionary<TABS.CarrierAccount, List<T>> zoneEntities = null;
                        if (!_ourEntities.TryGetValue(zone, out zoneEntities))
                        {
                            zoneEntities = new Dictionary<TABS.CarrierAccount, List<T>>();
                            _ourEntities[zone] = zoneEntities;
                        }
                        List<T> zoneCustomerEntities = null;
                        if (!zoneEntities.TryGetValue(customer, out zoneCustomerEntities))
                        {
                            zoneCustomerEntities = new List<T>();
                            zoneEntities[customer] = zoneCustomerEntities;
                        }
                        zoneEntities[customer].Add(entity);
                    }
                    // Supplier Entities
                    else
                    {
                        List<T> zoneEntities = null;
                        if (!_supplierEntities.TryGetValue(zone, out zoneEntities))
                        {
                            zoneEntities = new List<T>();
                            _supplierEntities[zone] = zoneEntities;
                        }
        ...
        DateSensitiveEntityCache<TABS.Rate> _rates;
        DateSensitiveEntityCache<TABS.ToDConsideration> _tod;
        DateSensitiveEntityCache<TABS.Commission> _commissions;
        DateSensitiveEntityCache<TABS.Tariff> _tariffs;
        protected DateTime _PricingStart = new DateTime(2000, 1, 1);

        public PricingGenerator(NHibernate.ISession session, DateTime pricingStart)
        {
            _PricingStart = pricingStart;
            _rates = new DateSensitiveEntityCache<Rate>(session, _PricingStart, false);
            _tod = new DateSensitiveEntityCache<ToDConsideration>(session, _PricingStart, false);
            _commissions = new DateSensitiveEntityCache<Commission>(session, _PricingStart, false);
            _tariffs = new DateSensitiveEntityCache<Tariff>(session, _PricingStart, false);
        }

        public PricingGenerator(NHibernate.ISession session, DateTime pricingStart, DateTime pricingRateStart)
        {
            _PricingStart = pricingStart;
            _rates = new DateSensitiveEntityCache<Rate>(session, pricingRateStart, true);
            _tod = new DateSensitiveEntityCache<ToDConsideration>(session, _PricingStart, true);
            _commissions = new DateSensitiveEntityCache<Commission>(session, _PricingStart, true);
            _tariffs = new DateSensitiveEntityCache<Tariff>(session, _PricingStart, true);
        }

        public List<Rate> GetRates(TABS.CarrierAccount customer, TABS.Zone zone, DateTime whenEffective)
        {
            return _rates.GetEffectiveEntities(customer, zone, whenEffective);
        }

        public List<ToDConsideration> GetToDConsiderations(TABS.CarrierAccount customer, TABS.Zone zone, DateTime whenEffective)
        {
            return _tod.GetEffectiveEntities(customer, zone, whenEffective);
        }

        public List<Commission> GetCommissions(TABS.CarrierAccount customer, TABS.Zone zone, DateTime whenEffective)
        {
            return _commissions.GetEffectiveEntities(customer, zone, whenEffective);
        }

        public List<Tariff> GetTariffs(TABS.CarrierAccount customer, TABS.Zone zone, DateTime whenEffective)
        {
            return _tariffs.GetEffectiveEntities(customer, zone, whenEffective);
        }
        #endregion Cacheable Entities

        /// <summary>
        /// Try to Fix Main Billing CDR when Our Zone is defined but no Sale is defined,
        /// by checking for a sale on a parent code.
        /// </summary>
        /// <param name="main">The main billing CDR</param>
        /// <param name="code">The code</param>
        /// <param name="originalZone">Internal use (recursion). set this to null.</param>
        /// <param name="codeMap">The code map used to find codes</param>
        /// <returns>True if Fixed, false if not fixed (or no need to)</returns>
        public bool FixParentCodeSale(Billing_CDR_Main main, Zone originalZone, Code code, CodeMap codeMap)
        {
    ...
    public class PricingGeneratorEntites<T> : IDisposable
    {
        ...
        public static List<T> Load(DateTime when, bool IsRepricing)
        {
            if (typeof(T) == typeof(TABS.Rate))
                return GetRates(when, IsRepricing) as List<T>;
            if (typeof(T) == typeof(TABS.Tariff))
                return GetTarrif(when) as List<T>;
            if (typeof(T) == typeof(TABS.ToDConsideration))
                return GetTOD(when) as List<T>;
            if (typeof(T) == typeof(TABS.Commission))
                return GetCommission(when) as List<T>;

            return new List<T>();
        }
        public static List<Commission> GetCommission(DateTime when)
        {
            ...
            sql.AppendFormat(@"SELECT [CommissionID],c.[SupplierID],[CustomerID],c.[ZoneID]
                                ...
                                order by z.Name ", when.ToString("yyyy-MM-dd"));
        ...
        public static List<TABS.ToDConsideration> GetTOD(DateTime when)
        {
        ...
        public static List<TABS.Tariff> GetTarrif(DateTime when)
        {
        ...
        public static List<TABS.Rate> GetRates(DateTime when, bool IsRepricing)
        {
