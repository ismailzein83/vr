...
namespace Tst
{
		...
		,ICloneable
	{
		...
		/// <exception cref="ArgumentException">An element with the same key already exists in the <see cref="TstDictionary"/>.
		/// </exception>
        ...
        public virtual ICollection PrefixSearch(string prefix)
        {
            if (prefix == null)
                throw new ArgumentNullException("key");
            if (prefix.Length == 0)
            ...
            ArrayList matches = new ArrayList();
            PrefixSearch(Root, prefix, 0, matches);
            return matches;
        }


        /// <summary>
        /// Search for all entries that have keys starting with the given prefix
        /// </summary>
        /// <param name="p">The starting Entry</param>
        /// <param name="prefix">The prefix to look for</param>
        /// <param name="index"></param>
        /// <param name="wildChar"></param>
        /// <param name="matches"></param>
        internal void PrefixSearch(
            TstDictionaryEntry p, 
			string prefix,
            int startIndex,
			IList matches
			)
		{
            ...
            char c = startIndex >= prefix.Length ? '\0' : prefix[startIndex];
            if (c == '\0' || c < p.SplitChar)
                PrefixSearch(p.LowChild, prefix, startIndex, matches);
            if (c == '\0' || c == p.SplitChar)
            {
                if (startIndex < prefix.Length - 1)
                    PrefixSearch(p.EqChild, prefix, startIndex + 1, matches);
                else
                {
                    if (p.IsKey)
                        matches.Add(new DictionaryEntry(p.Key, p.Value));
            ...
            if (c == '\0' || c > p.SplitChar)
                PrefixSearch(p.HighChild, prefix, startIndex, matches);
        }
