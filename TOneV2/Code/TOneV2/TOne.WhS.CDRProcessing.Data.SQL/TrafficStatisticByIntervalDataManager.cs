    ...
    public class TrafficStatisticByIntervalDataManager : BaseSQLDataManager, ITrafficStatisticByIntervalDataManager
    {
        ...
        public void UpdateStatisticItemsInDB(IEnumerable<TrafficStatisticByInterval> items)
        {
            ...
            if (dtStatisticsToUpdate.Rows.Count > 0)
                ExecuteNonQuerySPCmd("[TOneWhS_Analytic].[sp_TrafficStats_Update]",
                    (cmd) =>
                    {
        ...
        public Dictionary<string, long> GetStatisticItemsIdsByKeyFromDB(TrafficStatisticByIntervalBatch batch)
        {
            ...
            ExecuteReaderSP("[TOneWhS_Analytic].sp_TrafficStats_GetIdsByGroupedKeys", (reader) =>
            {
                string key = null;
                while (reader.Read())
                {
        ...
        private void WriteRecordToStream(TrafficStatisticByInterval record, object dbApplyStream)
        {
        ...
        private object FinishDBApplyStream(object dbApplyStream)
        {
            ...
            {
                ColumnNames = columns,
                TableName = "[TOneWhS_Analytic].[TrafficStats]",
        ...
        private void FillStatisticRow(DataRow dr, TrafficStatisticByInterval trafficStatistic)
        {
