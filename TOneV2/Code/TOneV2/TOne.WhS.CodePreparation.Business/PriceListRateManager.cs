...
namespace TOne.WhS.CodePreparation.Business
{
    public class PriceListRateManager
    {
        ...
        private HighestRate GetHighestRate(IEnumerable<ExistingRate> existingRates)
        {
            ...
            {
                Value = existingRates.ElementAt(0).RateEntity.Rate,
                CurrencyId = saleRateManager.GetCurrencyId(existingRates.ElementAt(0).RateEntity),
                BED = existingRates.ElementAt(0).RateEntity.BED,
                ZoneId = existingRates.ElementAt(0).ParentZone.ZoneId
            };

            for (int i = 1; i < existingRates.Count(); i++)
            {
                var existingRate = existingRates.ElementAt(i);
