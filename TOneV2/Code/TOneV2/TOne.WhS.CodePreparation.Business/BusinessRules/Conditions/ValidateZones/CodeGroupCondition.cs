...
namespace TOne.WhS.CodePreparation.Business
{
    public class CodeGroupCondition : BusinessRuleCondition
    {
        ...
        public override bool Validate(IBusinessRuleConditionValidateContext context)
        {
            ...
            {
                result = false;
                context.Message = string.Format("Code {0} not assigned to a code group", codeToAdd.Code);
            }
            else if (codeToClose != null && codeToClose.CodeGroup == null)
            {
                result = false;
                context.Message = string.Format("Code {0} not assigned to a code group", codeToClose.Code);
            }
