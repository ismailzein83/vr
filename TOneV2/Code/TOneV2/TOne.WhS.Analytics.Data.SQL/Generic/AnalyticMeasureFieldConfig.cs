...
namespace TOne.WhS.Analytics.Data.SQL
{
    ...
    public class MeasureValueExpression
    {
        ...
        public static MeasureValueExpression PGAD_Expression = new MeasureValueExpression { ColumnAlias = "Measure_PGAD", Expression = "CONVERT(DECIMAL(10,2),Sum(ts.SumOfPGAD)/(Sum(ts.SuccessfulAttempts)))", ExpressionSummary = AnalyticSummary.Avg.ToString("G") };
        public static MeasureValueExpression AveragePDD_Expression = new MeasureValueExpression { ColumnAlias = "Measure_AveragePDD", Expression = "CONVERT(DECIMAL(10,2),Sum(ts.SumOfPDDinSeconds)/(Sum(ts.SuccessfulAttempts))", ExpressionSummary = AnalyticSummary.Avg.ToString("G") };
