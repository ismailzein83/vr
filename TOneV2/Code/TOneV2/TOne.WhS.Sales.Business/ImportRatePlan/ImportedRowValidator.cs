...
namespace TOne.WhS.Sales.Business
{
	...
	public class OtherRateValidator : IImportedRowValidator
	{
		public bool IsValid(IIsValidContext context)
		{
			...
			else applicableRateTypeIds = GetApplicableRateTypeIdsForCustomerZone(context.OwnerId, context.ExistingZone.SaleZoneId);
		...
		private IEnumerable<int> GetApplicableRateTypeIdsForCustomerZone(int ownerId, long zoneId)
		{
			...
			target.TargetFieldValues.Add("CustomerId", ownerId);
			target.TargetFieldValues.Add("SaleZoneId", zoneId);
			target.EffectiveOn = DateTime.Now;
			return rateTypeRuleManager.GetRateTypes(new Guid("8A637067-0056-4BAE-B4D5-F80F00C0141B"), target);
		}
