        ...
        public override bool Validate(IBusinessRuleConditionValidateContext context)
        {
            ...
            if (zone.EED.HasValue)
            {
                if (zone.SaleZoneServiceToAdd != null)
                {
                    ...
                    {
                        string serviceEEDString = zone.SaleZoneServiceToAdd.EED.Value.Date.ToShortDateString();
                        context.Message = String.Format("EED '{0}' of the service of zone '{1}' must be less than or equal to EED '{2}' of the zone", serviceEEDString, zone.ZoneName, zone.EED.Value.ToShortDateString());
                        return false;
                    }
                }
                else if (zone.SaleZoneServiceToClose != null)
                {
                    ...
                    {
                        string serviceEEDString = zone.SaleZoneServiceToClose.CloseEffectiveDate.Date.ToShortDateString();
                        context.Message = String.Format("EED '{0}' of the service of zone '{1}' must be less than or equal to EED '{2}' of the zone", serviceEEDString, zone.ZoneName, zone.EED.Value.ToShortDateString());
                        return false;
                    }
                }
