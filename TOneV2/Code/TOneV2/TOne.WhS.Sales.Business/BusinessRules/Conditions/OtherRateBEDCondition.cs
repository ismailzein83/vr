            ...
            if (ratePlanContext.OwnerType == SalePriceListOwnerType.Customer)
            {
                ...
                foreach (var zoneData in allZoneData.DataByZoneList)
                {
                    if (zoneData.OtherRatesToChange.Count() > 0)
                    {
                        ...
                        var lastCustomerRate = customerZoneRateHistoryLocator.GetLastCustomerRateHistoryRecord(ratePlanContext.OwnerId, sellingProductId, zoneData.ZoneName, null, zoneData.CountryId, ratePlanContext.CurrencyId, ratePlanContext.LongPrecision);
                       
                        if (currentCustomerRate == null || lastCustomerRate == null)
                        ...
                        if (zoneData.OtherRatesToChange.First().BED < lastCustomerRate.BED && zoneData.NormalRateToChange.NormalRate == currentCustomerRate.Rate)
                        {
