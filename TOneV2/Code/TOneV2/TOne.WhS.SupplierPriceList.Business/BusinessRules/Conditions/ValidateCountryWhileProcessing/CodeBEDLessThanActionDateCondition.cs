    ...
    public class CodeBEDLessThanActionDateCondition : Vanrise.BusinessProcess.Entities.BusinessRuleCondition
    {
        ...
        public override bool Validate(Vanrise.BusinessProcess.Entities.IBusinessRuleConditionValidateContext context)
        {
            ...
            if (invalidCodes.Count > 0)
            {
                ...
                context.Message = string.Format("Adding code(s) to country '{0}' with BED less than '{1}'. Violated code(s): ({2}).", countryName,codeEffectiveDateString, string.Join(", ", invalidCodes));
                return false;
            }
