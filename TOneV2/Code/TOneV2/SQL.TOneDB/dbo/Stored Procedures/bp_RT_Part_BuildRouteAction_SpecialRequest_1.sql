...
CREATE PROCEDURE [dbo].[bp_RT_Part_BuildRouteAction_SpecialRequest]
...
EXEC bp_RT_Part_PrepareSpecialRequestRules
...
SELECT sr.CustomerID, sr.Code, sr.ZoneID,sr.SupplierOptions,sr.IncludeSubCodes,
 ...
 FROM #TempSpecialRequestRules sr WITH(NOLOCK)
	INNER JOIN #SaleZoneRates zr WITH(NOLOCK) ON sr.ZoneID = zr.ZoneID AND zr.CustomerID = sr.CustomerID COLLATE DATABASE_DEFAULT
 WHERE zr.IsBlock = 0	
...
AND rcp.Code COLLATE DATABASE_DEFAULT NOT IN (SELECT * FROM dbo.ParseArray(srt.Excludedcodes,',') pa)
...
SELECT 
 ...
 FROM (SpecialRequestWithSuppliers srt
INNER JOIN #Route_Special rs  WITH(NOLOCK) ON rs.RuleID = srt.RuleID)  
INNER JOIN #RouteOptionsPool cs  WITH(NOLOCK) ON cs.SupplierID = srt.SupplierID COLLATE DATABASE_DEFAULT AND  cs.Code COLLATE DATABASE_DEFAULT =  rs.Code


  SET @Message = CONVERT(varchar, getdate(), 121)
	EXEC bp_RT_Full_SetSystemMessages 'BuildRoutes: Special Request Route Options Inserted', @Message

CREATE NONCLUSTERED INDEX [IX_#RouteSpecial_mutikeys] ON [#Route_Special] ([RuleID],[Code]) INCLUDE ([CustomerID])	

---Write Back
DELETE rb FROM Route_Special rb
INNER JOIN #Route_Special  rbn ON rb.CustomerID COLLATE DATABASE_DEFAULT = rbn.CustomerID AND rb.Code COLLATE DATABASE_DEFAULT = rbn.Code

INSERT INTO Route_Special 
...
;WITH TempEmp 
    AS
    (
    SELECT ROW_NUMBER() OVER(PARTITION by  customerid, code, SupplierID ORDER BY customerid)
    AS duplicateRecCount
    FROM RouteOption_Special
    )
...
;WITH TempEmp 
    AS
    (
    SELECT ROW_NUMBER() OVER(PARTITION by  customerid, code, SupplierID ORDER BY RuleId)
    AS duplicateRecCount
...
SELECT 
 ...
 FROM #RouteOptionTemp srt
    ...
    DELETE rob FROM RouteOption_Special  rob
  INNER JOIN #RouteOption_Special  robn ON robn.CustomerID COLLATE DATABASE_DEFAULT = rob.CustomerID AND robn.Code COLLATE DATABASE_DEFAULT = rob.Code
  
  INSERT INTO RouteOption_Special 
  SELECT * FROM #RouteOption_Special  rob
--Testing
SELECT 'SpecialRoutes',* FROM #Route_Special rs
SELECT 'SpecialOptions',* FROM #RouteOption_Special ros	
