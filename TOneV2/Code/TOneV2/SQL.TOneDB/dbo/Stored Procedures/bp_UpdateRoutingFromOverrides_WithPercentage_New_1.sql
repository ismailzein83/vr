...
-- =================================================================
CREATE   PROCEDURE [dbo].[bp_UpdateRoutingFromOverrides_WithPercentage_New] 
...
BEGIN
	...
	CREATE TABLE #tmpRO (CustomerID nvarchar(5), Code nvarchar(15), IncludeSubCodes CHAR(1),ExcludedCodes VARCHAR(100),OurZoneID int, SupplierID nvarchar(5),BlockedSuppliers varchar(1024),Percentage INT, Position INT, SupplierZoneID INT NULL)
	CREATE TABLE #tmpRO_ZonesToCodes (CustomerID nvarchar(5), Code nvarchar(15), IncludeSubCodes CHAR(1),ExcludedCodes VARCHAR(100),OurZoneID int, SupplierID nvarchar(5),BlockedSuppliers varchar(1024), Percentage INT, Position INT, SupplierZoneID INT NULL)
				...
				Begin
					...
					Else
						Begin
							...
							if @OurZoneID <>-1
								UPDATE [RouteOption] SET [State] = @State_Blocked, Updated = GETDATE()
									FROM [Route] r WITH(NOLOCK), [RouteOption] WITH(NOLOCK, INDEX(IDX_RouteOption_RouteID)) 
									WHERE 
										r.CustomerID = @CustomerID
										AND r.OurZoneID = r.OurZoneID 
										AND [RouteOption].RouteID = r.RouteID
								...
								Begin
									...
									Else
										UPDATE [RouteOption] SET [State] = @State_Blocked, Updated = GETDATE()
											...
											WHERE 
												r.CustomerID = @CustomerID
												AND r.Code=@Code COLLATE Latin1_General_BIN
												AND [RouteOption].RouteID = r.RouteID
												AND [RouteOption].SupplierID IN (SELECT pa.[value] FROM dbo.ParseArray(@BlockedSuppliers, '|') pa)
								End
							COMMIT
