...
CREATE PROCEDURE [dbo].[bp_RT_Part_PrepareBlockRules]
...
EXEC bp_RT_Full_SetSystemMessages 'PartialBuildRoutes: START THE SP', @Message
...
,
BlockedRules AS ( SELECT * FROM BlockedRulesK WHERE ZONEID  IN ( SELECT ZONEID  FROM #CostZoneFilter) OR ZONEID  IN ( SELECT ZONEID  FROM #SaleZoneFilter) OR CODE COLLATE DATABASE_DEFAULT IN (SELECT CODE  COLLATE DATABASE_DEFAULT FROM #CodeFilter))
,
BlockCodeRulesWithoutDuplication AS(
SELECT	br.* 
FROM	BlockedRules br INNER JOIN 
...
--SELECT * FROM BlockCodeRulesWithoutDuplication WHERE customerid = 'c071' AND supplierid = 'c004'
,
...
FROM	BlockCodeRulesWithoutDuplication ro 
		LEFT JOIN #RouteOptionsPool cs  WITH(NOLOCK) ON cs.SupplierID COLLATE DATABASE_DEFAULT = ro.SupplierID AND cs.Code COLLATE DATABASE_DEFAULT  = ro.Code
...
SELECT 
		...
		--,ro.BeginEffectiveDate
FROM	BlockCodeRulesWithoutDuplication ro 
		LEFT JOIN #RouteOptionsPool cs  WITH(NOLOCK) ON cs.Code COLLATE DATABASE_DEFAULT  = ro.Code AND cs.SupplierID COLLATE DATABASE_DEFAULT = ro.SupplierID
		LEFT JOIN CodeGroup cg  WITH(NOLOCK) ON cg.Code = ro.Code
...
SET @Message = CONVERT(varchar, getdate(), 121)
EXEC bp_RT_Full_SetSystemMessages 'PartialBuildRoutes: PARENT Block Prepared Rules Inserted to Temp', @Message
...
FROM  #TempRules      tor LEFT JOIN 
            ParentRules par ON tor.CustomerID COLLATE DATABASE_DEFAULT = par.CustomerID aND par.SupplierID COLLATE DATABASE_DEFAULT  = tor.SupplierID
...
SET @Message = CONVERT(varchar, getdate(), 121)
EXEC bp_RT_Full_SetSystemMessages 'PartialBuildRoutes: Block Prepared Rules Inserted to Temp', @Message
...
DEALLOCATE DistinctRulesCursor
   ...
   SET @Message = CONVERT(varchar, getdate(), 121)
	EXEC bp_RT_Full_SetSystemMessages 'PartialBuildRoutes: Block Curser Finished', @Message
