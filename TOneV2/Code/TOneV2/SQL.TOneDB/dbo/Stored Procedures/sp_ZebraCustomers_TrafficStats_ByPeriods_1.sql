CREATE PROCEDURE [dbo].[sp_ZebraCustomers_TrafficStats_ByPeriods]
...
Traffic_Stats_Data AS (
	  ...
	  FirstCDRAttempt >= @FromDate
	   AND FirstCDRAttempt < @TillDate
	       AND ((@CustomerID IS NOT NULL AND ts.CustomerID IN (select * from dbo.fn_ado_param(@CustomerID,',')) ) OR (@AllAccounts IS NOT NULL AND ts.CustomerID IN (SELECT ParsedString FROM dbo.ParseStringList(@AllAccounts))) OR (@CustomerID IS NULL AND @AllAccounts IS NULL))
	...
	Traffic AS (
		...
		GROUP BY
			SupplierID, SupplierZoneID,CustomerID,
		...
		SELECT T.*,Z.Name,P.Name+ '(' + C.NameSuffix + ')' as SupplierName,P1.Name as CustomerName INTO #RESULT FROM Traffic T 
		...
		inner join CarrierAccount C1 on C1.CarrierAccountID=T.CustomerID
		inner join CarrierProfile P1 on C1.ProfileID=P1.ProfileID
		ORDER BY [Day]
...
Traffic_Stats_Data AS (
	...
	Traffic AS (
		...
		SELECT 
		 ...
		 GROUP BY
			ts.SupplierID, SupplierZoneID,CustomerID,
			CASE 
				WHEN @PeriodType like 'Days' THEN CONVERT(varchar(10), TS.FirstCDRAttempt, 121) 
				WHEN @PeriodType like 'Weeks' THEN 'Week ' + cast(datepart(wk,  TS.FirstCDRAttempt) AS varchar)
				WHEN @PeriodType like 'Months' THEN CONVERT(varchar(7), TS.FirstCDRAttempt, 121)
...
SELECT T.*,Z.Name,P.Name+ '(' + C.NameSuffix + ')' as SupplierName,P1.Name as CustomerName INTO #RESULT1 FROM Traffic T 
		inner join Zone Z on T.SupplierZoneID=Z.ZoneID
		inner join CarrierAccount C on C.CarrierAccountID=T.SupplierID
		inner join CarrierProfile P on C.ProfileID=P.ProfileID
		inner join CarrierAccount C1 on C1.CarrierAccountID=T.CustomerID
...
SET @SQLString = @SQLString + '
	select count(1),Sum(Attempts) as TAttempts,Sum(DurationsInMinutes) as TDurationsInMinutes,Sum(SuccessfulAttempts) as TSuccessfulAttempts
	,Sum(FailedAttempts) as TFailedAttempts,Sum(deliveredAttempts) as TdeliveredAttempts
	 from #result2
	 ;with FINAL AS 
                        (
