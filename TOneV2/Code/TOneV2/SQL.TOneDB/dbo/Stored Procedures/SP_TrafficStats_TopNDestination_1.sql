...
SET @Sql = @Sql + ' SELECT ' + convert(nvarchar(5),@TopRecord)
SET @Sql =@Sql+ ' SELECT TOP ' + convert(nvarchar(5),@TopRecord)
    ...
    CASE WHEN '''+@ShowSupplier+'''=''Y'' THEN S.SupplierID  ELSE NULL END AS SupplierID,
                    CASE WHEN ''' + ISNULL(@SupplierID,'') + ' '' = '''' THEN (CASE WHEN ''' + ISNULL(@SupplierCarrierGroupID,'') + ' ''= '''' THEN SUM(S.NumberOfCalls) ELSE SUM(S.Attempts) END) ELSE SUM(S.Attempts) END AS Attempts,
                    CONVERT(DECIMAL(10,2),SUM(DurationsInSeconds) / 60.0) as DurationsInMinutes,
                    CASE WHEN ''' + ISNULL(@SupplierID,'') + ' '' = '''' THEN (CASE WHEN ''' + ISNULL(@SupplierCarrierGroupID,'') + ' ''= '''' THEN (case when Sum(NumberOfCalls) > 0 then CONVERT(DECIMAL(10,2),Sum(SuccessfulAttempts)*100.0 / Sum(NumberOfCalls)) ELSE 0 end ) 
                    else (case when Sum(S.Attempts) > 0 then CONVERT(DECIMAL(10,2),Sum(SuccessfulAttempts)*100.0 / Sum(S.Attempts)) ELSE 0 end ) end) else (case when Sum(S.Attempts) > 0 then CONVERT(DECIMAL(10,2),Sum(SuccessfulAttempts)*100.0 / Sum(S.Attempts)) ELSE 0 end ) end AS ASR, 
                    ...
                    CASE WHEN ''' + ISNULL(@SupplierID,'') + ' '' = '''' THEN (CASE WHEN ''' + ISNULL(@SupplierCarrierGroupID,'') + ' ''= '''' THEN (case when Sum(NumberOfCalls) > 0 then CONVERT(DECIMAL(10,2),Sum(DeliveredNumberOfCalls) * 100.0 / SUM(NumberOfCalls)) ELSE 0 end ) 
                    else (case when Sum(S.Attempts) > 0 then CONVERT(DECIMAL(10,2),Sum(deliveredAttempts) * 100.0 / SUM(S.Attempts)) else 0 end ) end) else ( case when Sum(S.Attempts) > 0 then CONVERT(DECIMAL(10,2),Sum(deliveredAttempts) * 100.0 / SUM(S.Attempts)) else 0 end ) end  as DeliveredASR,
    ...
    SET @Sql = @Sql + ' LEFT JOIN CarrierAccount AS CA WITH (NOLOCK) ON (CASE when ''' +ISNULL(@SupplierID,'') + ' '' = '''' then (CASE WHEN ''' + ISNULL(@SupplierCarrierGroupID,'') + ' '' = '''' then ( S.CustomerID )else s.SupplierID end) else s.SupplierID end) = CA.CarrierAccountID  
                    WHERE
                        (
    ...
    BEGIN
        SET @Sql = @Sql + ' AND (s.CustomerID IN (SELECT CarrierAccountID from CarrierAccount where CarrierGroupID = '+@CarrierGroupID+')) '
    END
    IF @SupplierCarrierGroupID IS NOT NULL
    BEGIN
        SET @Sql = @Sql + ' AND (s.SupplierID IN (SELECT CarrierAccountID from CarrierAccount where CarrierGroupID = '+@SupplierCarrierGroupID+')) '
    END
    ...
    END
        SET @Sql = @Sql + ' ,CASE WHEN '''+@ShowSupplier+'''=''Y'' THEN S.SupplierID  ELSE NULL END
                        ORDER BY Attempts DESC, DurationsInMinutes '+ @sortOrder
