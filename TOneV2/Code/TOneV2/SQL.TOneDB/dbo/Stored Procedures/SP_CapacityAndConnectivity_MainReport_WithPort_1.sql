...
(
	...
	SELECT
			...
			NumberOfChannels_In = CASE WHEN @GroupByGateWay = 'Y' THEN AVG(DC.NumberOfChannels_In) 
								       WHEN @GroupByGateWay = 'N' THEN (SELECT SUM(CSC.NumberOfChannels_In) FROM InterConnect CSC WHERE CSC.Date = 	DATEADD(day,0,datediff(day,0, FirstCDRAttempt)) )
								       ELSE 0 END,
			NumberOfChannels_Out = CASE WHEN @GroupByGateWay = 'Y' THEN AVG(DC.NumberOfChannels_Out) 
								       WHEN @GroupByGateWay = 'N' THEN (SELECT SUM(CSC.NumberOfChannels_Out) FROM InterConnect CSC WHERE CSC.Date = 	DATEADD(day,0,datediff(day,0, FirstCDRAttempt)) )
								       ELSE 0 END,
			NumberOfChannels_Total = CASE WHEN @GroupByGateWay = 'Y' THEN AVG(DC.NumberOfChannels_Total) 
								       WHEN @GroupByGateWay = 'N' THEN (SELECT SUM(CSC.NumberOfChannels_Total) FROM InterConnect CSC WHERE CSC.Date = 	DATEADD(day,0,datediff(day,0, FirstCDRAttempt)) )
								       ELSE 0 END
	   FROM #CarrierTraffic AS TS -- WITH(NOLOCK,INDEX(IX_TrafficStats_DateTimeFirst))
		--	LEFT JOIN Zone AS Z WITH(NOLOCK) ON TS.OurZoneID = Z.ZoneID  
			LEFT JOIN @Connectivities DC ON  DC.Date = DATEADD(day,0,datediff(day,0, FirstCDRAttempt)) AND DC.InterconnectedSwithes = 'N'
	   ...
	   WHERE   
				...
				1 = (CASE  WHEN @SelectPort  = 0 AND TS.CustomerID = @CarrierAccountID AND TS.Port_In like @PortInOut THEN 1     
...
SELECT
	   ...
	   FROM #CarrierTraffic AS TS 
		--	LEFT JOIN Zone AS Z WITH(NOLOCK) ON TS.OurZoneID = Z.ZoneID  
			LEFT JOIN @Connectivities DC ON  DC.Date = DATEADD(day,0,datediff(day,0, ts.FirstCDRAttempt)) AND DC.InterconnectedSwithes = 'N'
									  AND (@GateWayName IS NULL OR (Dc.gateway = @GateWayName)) 
		    LEFT JOIN @Connectivities DCI ON  DCI.Date = DATEADD(day,0,datediff(day,0, ts.FirstCDRAttempt)) AND DCI.InterconnectedSwithes = 'Y'	                                   
	   WHERE   
	   ts.attempts>0 and 
	1 = (CASE  WHEN @SelectPort  = 0 AND TS.CustomerID = @CarrierAccountID AND TS.Port_In like @PortInOut THEN 1     
		                   WHEN @SelectPort  = 1 AND TS.SupplierID = @CarrierAccountID AND TS.Port_Out like @PortInOut THEN 1 
