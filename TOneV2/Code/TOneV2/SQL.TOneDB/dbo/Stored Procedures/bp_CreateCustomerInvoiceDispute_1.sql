...
CREATE PROCEDURE [dbo].[bp_CreateCustomerInvoiceDispute]
(
...
SET @Pivot = @FromDate
   ...
   INTO   #TempBillingCDR
FROM   [DATABASENAME].dbo.Billing_CDR_Main bcm WITH(NOLOCK)
       LEFT JOIN [DATABASENAME].dbo.Billing_CDR_Sale bcs (NOLOCK) ON  bcs.ID = bcm.ID
...
(
  SELECT
       *  
  FROM Billing_CDR_Sale bcs WITH(NOLOCK),BillingMainIDCTE 
  WHERE bcs.Attempt >= @FromDate
	AND  bcs.Attempt <= @ToDate
	And bcs.ID >= BillingMainIDCTE.MinID
	AND bcs.ID <= BillingMainIDCTE.MaxID
)
