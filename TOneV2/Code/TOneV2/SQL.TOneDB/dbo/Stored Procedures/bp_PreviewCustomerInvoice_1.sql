...
,SaleCTE AS (SELECT * FROM Billing_CDR_Sale bcs  WITH(NOLOCK)
...
SELECT 
       ...
       @Amount = ISNULL(Round(SUM(Amount),2),0),
	   --SUM(Amount),
...
(
    ...
    Duration, Amount, NumberOfCalls, CurrencyID, IsLocked, IsPaid, UserID,CreationDate
)
...
SELECT @Amount = ISNULL(Round(SUM(B.amount / ISNULL(ERS.Rate, 1)),2),0)  
...
SET    Duration = ISNULL(@Duration,0),
	   ...
	   Amount = ISNULL(Round(@Amount,2),0),
