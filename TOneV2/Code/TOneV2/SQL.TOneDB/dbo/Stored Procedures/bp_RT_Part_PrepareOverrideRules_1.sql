...
CREATE PROCEDURE [dbo].[bp_RT_Part_PrepareOverrideRules]
...
FROM	RouteOverride ro  WITH(NOLOCK) 
		LEFT JOIN #CustomerFilter ca  WITH(NOLOCK) ON ca.CustomerID COLLATE DATABASE_DEFAULT = ro.CustomerID		 
		--LEFT JOIN Zone z  WITH(NOLOCK) ON z.CodeGroup = ro.Code
		INNER JOIN #SaleZoneFilter szf ON szf.CodeGroup COLLATE DATABASE_DEFAULT = ro.Code
...
FROM	RouteOverride ro  WITH(NOLOCK) 
		...
		LEFT JOIN #SaleZoneRates zr  WITH(NOLOCK) ON zr.ZoneID = ro.OurZoneID AND zr.CustomerID COLLATE DATABASE_DEFAULT = ro.CustomerID -- To check if SYS sell this zone to the customer
...
FROM	RouteOverride ro  WITH(NOLOCK) 
		LEFT JOIN #CustomerFilter ca  WITH(NOLOCK) ON ca.CustomerID COLLATE DATABASE_DEFAULT = ro.CustomerID
		LEFT JOIN #RoutePool rcp  WITH(NOLOCK) ON rcp.Code COLLATE DATABASE_DEFAULT = ro.Code
...
WHERE	ro.IsEffective = 'Y' AND ro.RouteOptions IS NOT NULL
		...
		AND ro.IncludeSubCodes = 'N' --OR (ro.IncludeSubCodes = 'Y' AND ro.Code NOT IN (SELECT cg.Code FROM CodeGroup cg))) -- Code-Rule: Not a CG-Rule		
...
FROM	RouteOverride ro  WITH(NOLOCK) 
		LEFT JOIN #CustomerFilter ca  WITH(NOLOCK) ON ca.CustomerID COLLATE DATABASE_DEFAULT = ro.CustomerID
		LEFT JOIN #RoutePool rcp  WITH(NOLOCK) ON rcp.Code IS NOT NULL
...
WHERE	ro.IsEffective = 'Y' AND ro.RouteOptions IS NOT NULL
		...
		AND rcp.Code IS NOT NULL AND rcp.ZoneID IS NOT NULL		
		 AND rcp.Code COLLATE DATABASE_DEFAULT LIKE ro.Code + '%'
...
(
	...
	WHERE  scg.RowNumber = 1 AND NOT EXISTS (
		...
		WHERE srz.code = scg.code COLLATE DATABASE_DEFAULT  AND srz.CustomerID = scg.CustomerID COLLATE DATABASE_DEFAULT )
)
,
