...
CREATE PROCEDURE [dbo].[bp_RT_Full_BuildRouteAction_Override]

WITH RECOMPILE
AS
BEGIN

DROP INDEX [IX_RouteOption_Override_multikey] ON [RouteOption_Override]
...
exec  bp_RT_Full_PrepareOverrideRules
...
SELECT 
 ...
 INNER JOIN ZoneRates zr WITH(NOLOCK) ON tor.OurZoneID = zr.ZoneID AND zr.CustomerID = tor.CustomerID COLLATE DATABASE_DEFAULT
...
WHERE tmp.code IS NULL AND rcp.Code NOT IN (SELECT * FROM dbo.ParseArray(tmp.Excludedcodes,',') pa)
...
INNER JOIN RouteOptionsPool cs WITH(NOLOCK) ON cs.SupplierID = rov.SupplierID AND ro.Code = cs.Code
...
SELECT 
 ...
 FROM (#RouteOverrideWithSuppliers rov WITH(NOLOCK)
INNER JOIN Route_Override ro WITH(NOLOCK) ON ro.RuleID = rov.RuleID)  
INNER JOIN RouteOptionsPool cs WITH(NOLOCK) ON ro.Code = cs.Code
	WHERE rov.SupplierID = 'BLK'
