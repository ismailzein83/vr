	create PROCEDURE [dbo].[bp_BuildRoutes_RoutingPool]
...
BEGIN
	...
	SET @RouteOptionIndexesCreationSQL = '
		CREATE NONCLUSTERED INDEX [IDX_RouteOption_RouteID] ON [dbo].[TempRouteOption]([RouteID] ASC) WITH (SORT_IN_TEMPDB = '+@SORT_IN_TEMPDB+') 
...
'
	SET @RouteOptionIndexesCreationSQL1 = '
		CREATE NONCLUSTERED INDEX [IDX_RouteOption_RouteID] ON [dbo].[TempRouteOption]([RouteID] ASC) WITH (ALLOW_ROW_LOCKS  = ON, ALLOW_PAGE_LOCKS  = ON,SORT_IN_TEMPDB = '+@SORT_IN_TEMPDB+') 
	';
	...
	EXEC (@RouteOptionTableCreationSQL);
    	...
    	,CodeMatchTemp AS (
    	 ...
    	 , RoutesToInsert AS (		
			SELECT
				convert(int,ROW_NUMBER() OVER (ORDER BY R.CustomerID,CM.Code)) AS RouteID,
...
--	EXEC bp_SetSystemMessage 'BuildRoutes: INDEX [PK_RouteID] Built for Routes', @Message;
	...
	(
		...
		FROM
				...
				and (RPS.ZoneID<=0 or cs.SupplierZoneID=RPS.ZoneID)
			WHERE 
