...
Declare @Traffic TABLE (SupplierID VarChar(15), Attempts int, DurationsInMinutes numeric(13,5), SuccessfulAttempts numeric (13,5),ASR numeric(13,5), ACD numeric(13,5), DeliveredASR numeric(13,5), AveragePDD numeric(13,5))
...
SELECT 
    ...
    WHERE (FirstCDRAttempt >= @FromDate AND FirstCDRAttempt <= @ToDate)
                  AND (TS.CustomerID = @CustomerID) AND ts.CustomerID NOT IN ( SELECT grasc.CID FROM dbo.GetRepresentedAsSwitchCarriers() grasc)
...
DECLARE @Result TABLE (
            SupplierID VarChar(15),Attempts int, AttemptPercentage DECIMAL(13,5), DurationsInMinutes numeric(13,5), SuccessfulAttempts numeric (13,5),ASR numeric(13,5), ACD numeric(13,5), DeliveredASR numeric(13,5), AveragePDD numeric(13,5),
            NumberOfCalls int,Cost_Nets float, Sale_Nets float,Profit numeric (13,5),AverageSaleRate NUMERIC(13,5),AverageCostRate NUMERIC(13,5),Percentage numeric (13,5) , PricedDuration  DECIMAL(13,5)
            )
begin
...
INSERT INTO @Result(SupplierID ,Attempts, DurationsInMinutes,SuccessfulAttempts, ASR, ACD, DeliveredASR, AveragePDD, NumberOfCalls, Cost_Nets, Sale_Nets, Profit,AverageSaleRate,AverageCostRate, Percentage, PricedDuration)
      SELECT 
              TS.SupplierID, TS.Attempts, TS.DurationsInMinutes,TS.SuccessfulAttempts,TS.ASR, TS.ACD, TS.DeliveredASR, TS.AveragePDD,
				 BS.Calls, BS.Cost,
				  ...
				  BS.PricedDuration           
    FROM @Traffic TS 
            LEFT JOIN MyBilling BS WITH (NOLOCK,Index(IX_Billing_Stats_Date,IX_Billing_Stats_Customer)) ON TS.SupplierID= BS.SupplierID 
