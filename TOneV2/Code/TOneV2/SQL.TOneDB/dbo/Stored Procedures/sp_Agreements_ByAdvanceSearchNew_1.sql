...
CREATE PROCEDURE [dbo].[sp_Agreements_ByAdvanceSearchNew]
(
...
BEGIN
	...
	CarrierTable as 
	(
		SELECT	CASE WHEN  @ShowNameSuffix ='Y' THEN case when A.NameSuffix!='' THEN  P.Name+'('+A.NameSuffix+')' else P.Name end ELSE P.Name END  AS CarrierName,A.CarrierAccountID 
		FROM	CarrierAccount A WITH(NOLOCK)
				inner join CarrierProfile P WITH(NOLOCK) ON P.ProfileID=A.ProfileID
	 )
	SELECT  bh.*,ca.CarrierName as carrierName,
			CASE bh.AgreementType WHEN 0 THEN 'Gentlemen Agreement' WHEN 1 THEN 'Commitment Agreement' END AS AgreementName ,
			CASE bh.ContractType WHEN 0 THEN 'Balanced Amount' WHEN 1 THEN 'Balanced Duration' WHEN 2 THEN 'Un Balanced' END AS ContractName
    INTO	#RESULT
	FROM	BilateralAgreement bh  WITH(NOLOCK)
			LEFT JOIN CarrierTable ca ON ca.CarrierAccountID= bh.CarrierID
	WHERE bh.BeginDate BETWEEN ISNULL(@FromBED,bh.BeginDate) AND ISNULL(@ToBED,bh.BeginDate)  
          ...
          AND bh.[Description] = ISNULL('%'+@Description+'%',bh.[Description]) 
	...
	SELECT	bad.[ID]
			...
			,bad.[MinSaleRate]
			,bad.[SubstituteRate]
			,bad.[IsRoutingRulesApplied]
			,bad.[OverrideActionApplied]
	FROM	BilateralAgreementDetails bad WITH(NOLOCK)
			INNER JOIN BilateralAgreement ba WITH(NOLOCK) ON ba.ID = bad.AgreementID
	WHERE	ba.ID IN (SELECT TOP ' + CONVERT(VARCHAR(4),@Top) +' t.ID FROM #resultFINAL t WHERE t.rowNumber between '+ CAST( @From AS varchar) +' AND  '+CAST( @To as varchar)+')'
