...
namespace TOne.WhS.BusinessEntity.Business
{
	public class SaleEntityServiceManager
	{
		...
		{
			ISaleEntityServiceDataManager dataManager = BEDataManagerFactory.GetDataManager<ISaleEntityServiceDataManager>();
			var allSaleZoneRoutingProducts = dataManager.GetSaleZoneRoutingProductsEffectiveAfter(sellingNumberPlanId, effectiveOn);
			var filteredSaleZoneRoutingProducts = new List<SaleZoneRoutingProduct>();
			CarrierAccountManager carrierAccountManager = new CarrierAccountManager();
			foreach (var zoneRoutingProduct in allSaleZoneRoutingProducts)
			{
				if (!(zoneRoutingProduct.OwnerType == SalePriceListOwnerType.Customer && carrierAccountManager.IsCarrierAccountDeleted(zoneRoutingProduct.OwnerId)))
					filteredSaleZoneRoutingProducts.Add(zoneRoutingProduct);
			}

			return filteredSaleZoneRoutingProducts;
		}
