...
{
    public class RoutingProductManager : BaseBusinessEntityManager
    {
            ...
            return Vanrise.Caching.CacheManagerFactory.GetCacheManager<SaleEntityRoutingProductCacheManager>()
                .GetOrCreateObject("GetAllCachedSaleZoneRoutingProducts", () =>
                {
                    ...
                    if (allSaleZoneRoutingProducts != null)
                    {
                        ...
                        {
                            if (szRP.OwnerType != SalePriceListOwnerType.Customer ||
                                !carrierAccountManager.IsCarrierAccountDeleted(szRP.OwnerId))
                                rslt.GetOrCreateItem(new RoutingProductOwnerKey
                                { OwnerId = szRP.OwnerId, OwnerType = szRP.OwnerType })
                                    .GetOrCreateItem(szRP.SaleZoneId).Add(szRP);
                        }
                    }
                    ...
                    if (allSaleZoneRoutingProducts != null)
                    {
                        foreach (var szRP in allSaleZoneRoutingProducts)
                        {
                            if (szRP.OwnerType != SalePriceListOwnerType.Customer ||
                                !carrierAccountManager.IsCarrierAccountDeleted(szRP.OwnerId))
                                rslt.GetOrCreateItem(new RoutingProductOwnerKey
                                { OwnerId = szRP.OwnerId, OwnerType = szRP.OwnerType }).Add(szRP);
                        }
                    }
            ...
            Vanrise.Entities.DataRetrievalInput<RoutingProductQuery> input)
        {
            ...
            Func<RoutingProduct, bool> filterExpression = (prod) =>
                ...
                &&
                (input.Query.SellingNumberPlanIds == null ||
                 input.Query.SellingNumberPlanIds.Contains(prod.SellingNumberPlanId));
        ...
        public IEnumerable<RoutingProductInfo> GetRoutingProductsInfoBySellingNumberPlan(int sellingNumberPlanId)
        {
            ...
            return routingProducts.MapRecords(RoutingProductInfoMapper,
                x => x.SellingNumberPlanId == sellingNumberPlanId);
        }
        ...
        public IEnumerable<RoutingProductInfo> GetRoutingProductInfo(RoutingProductInfoFilter filter)
        {
            ...
            if (filter != null)
            {
                ...
                else if (filter.AssignableToOwnerType.HasValue)
                {
                    ...
                    {
                        filterAssignableToSaleEntity = (rp) =>
                            (rp.SellingNumberPlanId == sellingNumberPlanId.Value && rp.Settings != null &&
                             rp.Settings.ZoneRelationType == RoutingProductZoneRelationType.AllZones);
                    }
                }
        ...
        public UpdateOperationOutput<RoutingProductDetail> UpdateRoutingProduct(RoutingProductToEdit routingProduct)
        {
            if (CheckIfRoutingProductHasRelatedSaleEntities(routingProduct.RoutingProductId))
            {
                ...
                if (currentRoutingProduct.Settings.ZoneRelationType != routingProduct.Settings.ZoneRelationType ||
                    ...
                    throw new Vanrise.Entities.VRBusinessException(
                        string.Format("Cannot edit Routing Product '{0}' because it is in use.", routingProduct.Name));
            }
        ...
        private void GetZoneServicesData(List<RoutingProductZoneService> zoneServices, out HashSet<long> zoneIds,
            out HashSet<int> serviceIds)
        {
                        ...
                        switch (routingProduct.Settings.ZoneRelationType)
                        {
                            ...
                            case RoutingProductZoneRelationType.SpecificZones:
                                if (routingProduct.Settings.Zones == null)
                                    ...
                                    filteredZoneIds =
                                        new HashSet<long>(routingProduct.Settings.Zones.Select(zone => zone.ZoneId));
                                break;
                        }
                    }
                        ...
                        switch (routingProduct.Settings.SupplierRelationType)
                        {
                            ...
                            case RoutingProductSupplierRelationType.SpecificSuppliers:
                                if (routingProduct.Settings.Suppliers == null)
                                    ...
                                    filteredSupplierIds = new HashSet<int>(
                                        routingProduct.Settings.Suppliers.Select(supplier => supplier.SupplierId));
                                break;
                        }
                    }
            ...
            return Vanrise.Caching.CacheManagerFactory.GetCacheManager<CacheManager>().GetOrCreateObject(
                ...
                () =>
                {
                    IRoutingProductDataManager dataManager =
                        BEDataManagerFactory.GetDataManager<IRoutingProductDataManager>();
        ...
        {
            public override void ConvertResultToExcelData(
                IConvertResultToExcelDataContext<RoutingProductDetail> context)
            {
            ...
            public Dictionary<int, RoutingProducts> GetAllRoutingProductsBySellingNumberPlanId()
            {
                ...
                return Vanrise.Caching.CacheManagerFactory.GetCacheManager<CacheManager>().GetOrCreateObject(
                    "GetRoutingProductsBySellingNumberPlan", () =>
                    {
                        ...
                        Dictionary<int, RoutingProducts> routingProductsByNumberPlan =
                            new Dictionary<int, RoutingProducts>();
                        foreach (RoutingProduct routingProduct in routingProducts)
                        {
                            RoutingProducts currentRoutingProducts;
                            if (!routingProductsByNumberPlan.TryGetValue(routingProduct.SellingNumberPlanId,
                                out currentRoutingProducts))
                            {
                                currentRoutingProducts = new RoutingProducts();
                                routingProductsByNumberPlan.Add(routingProduct.SellingNumberPlanId,
                                    currentRoutingProducts);
                            }
                            ...
                            foreach (var saleZone in routingProduct.Settings.Zones)
                            {
                                    ...
                                    out routingProductsWithSpecificZones))
                                {
                                    ...
                                    RoutingProductIdsWithSpecificZones.Add(saleZone.ZoneId,
                                        routingProductsWithSpecificZones);
                                }
        ...
        {
            var routingProducts = GetAllRoutingProducts();
            return routingProducts?.Select(itm => itm as dynamic).ToList();
        }

        public override dynamic GetEntity(IBusinessEntityGetByIdContext context)
        {
            return GetRoutingProduct(Convert.ToInt32(context.EntityId));
        }

        public override dynamic GetEntityId(IBusinessEntityIdContext context)
        {
            var routingProduct = context.Entity as RoutingProduct;
            return routingProduct.RoutingProductId;
        }

        public override string GetEntityDescription(IBusinessEntityDescriptionContext context)
        {
            return GetRoutingProductName(Convert.ToInt32(context.EntityId));
        }
