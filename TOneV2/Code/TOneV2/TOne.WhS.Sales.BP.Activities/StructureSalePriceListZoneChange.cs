...
namespace TOne.WhS.Sales.BP.Activities
{
	public class StructureSalePriceListZoneChange : CodeActivity
	{
		...
		private List<CustomerPriceListChange> GetChangesForSellingProduct(SellingProductChangesContext context, out List<SalePricelistRPChange> outRoutingProductChanges)
		{
			...
			foreach (var customer in context.Customers)
			{
				...
				List<SalePricelistRateChange> rateChanges = this.GetRateChangesForCustomer(customer.CustomerId, customer.SellingProductId, soldCountries,
					context.ImportedZonesByCountryId, context.LastRateNoCachelocator, context.Futurelocator, saleRateManager);
		...
		private List<SalePricelistRateChange> GetRateChangesForCustomer(int customerId, int sellingProductId, IEnumerable<CustomerCountry2> soldCountries,
			Dictionary<int, List<DataByZone>> importedZonesByCountryId, SaleEntityZoneRateLocator lastRateNoCachelocator, SaleEntityZoneRateLocator futurelocator, SaleRateManager saleRateManager)
		{
			...
			foreach (var country in soldCountries)
			{
				...
				foreach (var zone in zones)
				{
					...
					if (zone.NormalRateToChange != null && zone.NormalRateToChange.RateTypeId == null)
					{
						...
						{
							salePricelistRateChange.RecentCurrencyId = saleRateManager.GetCurrencyId(zone.NormalRateToChange.RecentExistingRate.RateEntity);
							salePricelistRateChange.RecentRate = zone.NormalRateToChange.RecentExistingRate.RateEntity.Rate;
							salePricelistRateChange.RecentRateConverted = zone.NormalRateToChange.RecentExistingRate.ConvertedRate;
						}
			...
			foreach (var rateToChange in context.StructuredRateActions.RatesToChange)
			{
				...
				if (rateToChange.OtheRates != null)
				{
					...
					foreach (var otheRateToChange in rateToChange.OtheRates)
					{
						...
						salePricelistOtherRateChange.RecentCurrencyId = saleRateManager.GetCurrencyId(otheRateToChange.RecentExistingRate.RateEntity);
						salePricelistOtherRateChange.RecentRate = otheRateToChange.RecentExistingRate.RateEntity.Rate;
						salePricelistOtherRateChange.RecentRateConverted = otheRateToChange.RecentExistingRate.ConvertedRate;
			...
			foreach (var rateToClose in context.StructuredRateActions.RatesToClose)
			{
				...
				if (normalRateToClose != null)
				{
					var salePriceListRateChange = new SalePricelistRateChange
					{
						...
						RecentCurrencyId = saleRateManager.GetCurrencyId(recentRate.Rate),
						RecentRateConverted = UtilitiesManager.ConvertToCurrencyAndRound(recentRate.Rate.Rate,
				...
				foreach (var otheRateToChange in otherRatesToClose)
				{
					...
					if (sellingProductRate.RatesByRateType.TryGetValue(otheRateToChange.RateTypeId.Value, out sellingProductOtherRate))
					{
						var salePricelistOtherRateChange = new SalePricelistRateChange
						{
							...
							RecentRate = customerOtherRate.Rate,
							RecentCurrencyId = saleRateManager.GetCurrencyId(customerOtherRate),
							RecentRateConverted = UtilitiesManager.ConvertToCurrencyAndRound(customerOtherRate.Rate,
