...
namespace TOne.BusinessEntity.Data.SQL
{
    ...
    {
        public Vanrise.Entities.BigResult<SupplierCommission> GetSupplierCommissions(Vanrise.Entities.DataRetrievalInput<SupplierCommissionQuery> input)
       {
           Action<string> createTempTableAction = (tempTableName) =>
           {
               ExecuteNonQuerySP("BEntity.sp_Commision_CreateTempBySupplier", tempTableName, input.Query.SupplierId, input.Query.ZoneId, input.Query.EffectiveFrom);
           };
           return RetrieveData(input, createTempTableAction, CommissionTempTableMapper);
       }
       public List<SupplierCommission> GetSupplierCommissions(DateTime when)
       {
           return GetItemsSP("BEntity.sp_Commission_GetCost", CommissionMapper, when);
       }

       private SupplierCommission CommissionMapper(IDataReader reader)
       {
           SupplierCommission commission = new SupplierCommission();
       ...
       private SupplierCommission CommissionTempTableMapper(IDataReader reader)
       {
           SupplierCommission commission = new SupplierCommission();
           ...
           commission.SupplierName = reader["SupplierName"] as string;
