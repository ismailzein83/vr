    ...
    function RatePlanUtilsService(UtilsService, VRValidationService, VRDateTimeService, WhS_BE_SalePriceListOwnerTypeEnum, WhS_BE_PrimarySaleEntityEnum, UISettingsService, $filter) {
        ...
        function setRateChangeTypeIcon(dataItem) {
            if (dataItem.NewRate) // This check is false when newRate is undefined, null or an empty string
            {
                ...
                else if (Number(dataItem.NewRate.replace(/,/g, '')) > dataItem.CurrentRate) {
        ...
        function validateNewRate(dataItem, ownerCurrencyId, allowRateZero) {
            var newRate = parseFloat(dataItem.NewRate);
            if (allowRateZero && newRate < 0)
                return 'New rate must be greater than or equal to 0';
            if (!allowRateZero && newRate <= 0)
        ...
        function validateNewRateDates(dataItem) {
            ...
            var maxOtherRateDate = undefined;
            if (dataItem.CurrentOtherRates != null) {
                var otherRateDates = [];
                //otherRatesDates.length = 0;
                for (var key in dataItem.CurrentOtherRates) {
                    if (key != "$type")
                        otherRateDates.push(UtilsService.createDateFromString(dataItem.CurrentOtherRates[key].BED));
                }
                maxOtherRateDate = this.getMaxDate(otherRateDates);
            }

            if (maxOtherRateDate != undefined && maxOtherRateDate > minRateBED)
                minRateBED = maxOtherRateDate;
