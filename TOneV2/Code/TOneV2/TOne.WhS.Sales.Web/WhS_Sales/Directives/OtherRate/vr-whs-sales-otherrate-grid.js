...
app.directive("vrWhsSalesOtherrateGrid", ["UtilsService", "VRNotificationService", 'WhS_Sales_RatePlanUtilsService', 'WhS_Sales_RatePlanService', 'WhS_Sales_RateSourceEnum', 'VRDateTimeService', 'WhS_Sales_SalePriceListOwnerTypeEnum', 'UISettingsService', '$filter', function (UtilsService, VRNotificationService, WhS_Sales_RatePlanUtilsService, WhS_Sales_RatePlanService, WhS_Sales_RateSourceEnum, VRDateTimeService, WhS_Sales_SalePriceListOwnerTypeEnum, UISettingsService, $filter) {
    return {
        restrict: "E",
        scope: {
            onReady: "="
    ...
    function OtherRateGrid($scope, ctrl, $attrs) {
        ...
        function initializeController() {
            ...
            $scope.areOtherRatesEditable = function () {
                if (zoneItem.CurrentRate != null)
                    return true;
                if (zoneItem.NewRate != null && WhS_Sales_RatePlanUtilsService.validateNewRate(zoneItem, ownerCurrencyId, allowRateZero) == null)
                    return true;
                return false;
            };
            ...
            $scope.validateNewRateDates = function (dataItem) {
                if (dataItem.CurrentRateNewEED != undefined && dataItem.CurrentRateNewEED != '' && dataItem.CurrentRateNewEED != null) {
                    if (dataItem.CurrentRateNewEED < UtilsService.createDateFromString(dataItem.CurrentRateBED))
                        return "Other rate EED should be greater than other rate BED.";
                }
                return null;
            };
            ...
            $scope.validateNewRate = function (otherRate) {
                return WhS_Sales_RatePlanUtilsService.validateNewRate(otherRate, ownerCurrencyId, allowRateZero);
            };
            ...
            $scope.validateNewRateBED = function () {
                ...
                if ($scope.NewRateBED != undefined) {
                    if (zoneItem.CurrentRateEED != undefined && $scope.NewRateBED < currentEED.getTime() && zoneItem.NewRateBED == undefined) {
                        return "BED of the other rate must be greater than or equal to '" + UtilsService.getShortDate(UtilsService.createDateFromString(zoneItem.CurrentRateEED)) + "' BED of the last normal rate of the zone";
                    }

                    if ($scope.otherRates != undefined && $scope.otherRates.length > 0) {
                        var otherRateDates = $scope.otherRates.map(item => UtilsService.createDateFromString(item.CurrentRateBED));
                        var maxOtherRateDate = WhS_Sales_RatePlanUtilsService.getMaxDate(otherRateDates);
                        if ($scope.NewRateBED != undefined && $scope.NewRateBED != '' && $scope.NewRateBED != null && UtilsService.createDateFromString($scope.NewRateBED) < maxOtherRateDate) {
                            return "BED of the other rate must be greater than or equal to '" + UtilsService.getShortDate(maxOtherRateDate) + "' BED of the last other rate of the zone";
                        }
                    }

                    if (zoneItem.CurrentRateBED != undefined && $scope.NewRateBED < UtilsService.createDateFromString(zoneItem.CurrentRateBED) && zoneItem.NewRateBED == undefined) {
                        return "BED of the other rate must be greater than or equal to '" + UtilsService.getShortDate(UtilsService.createDateFromString(zoneItem.CurrentRateBED)) + "' BED of the normal rate of the zone";
                    }
                }
                else
                    return null;
        ...
        function defineAPI() {
            ...
            api.loadGrid = function (query) {
                ...
                allowRateZero = (query.saleAreaSetting != undefined && query.saleAreaSetting.PricingSettings != undefined) ? query.saleAreaSetting.PricingSettings.AllowRateZero : false;
            ...
            function loadGrid() {
                ...
                for (var i = 0; i < zoneItem.RateTypes.length; i++) {
                    ...
                    if (newOtherRate != null) {
                        zoneItem.IsDirty = true;
                        otherRate.NewRate = $filter('vrtextOrNumber')(newOtherRate.Rate, $scope.longPrecision);
