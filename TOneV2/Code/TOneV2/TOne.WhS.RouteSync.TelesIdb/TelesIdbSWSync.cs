...
using TOne.WhS.RouteSync.TelesIdb.Business;
...
namespace TOne.WhS.RouteSync.TelesIdb
{
	public class TelesIdbSWSync : SwitchRouteSynchronizer
	{
		...
		public override void ConvertRoutes(ISwitchRouteSynchronizerConvertRoutesContext context)
		{
			...
			#region Create manualRouteIterator
			RoutePrefIterator manualRouteIterator = null;
			if (ManualRoutes != null && ManualRoutes.Any())
			{
				List<string> allPrefs = new List<string>();
				List<string> includeSubCodesPref = new List<string>();
				foreach (ManualRoute manualRoute in ManualRoutes)
				{
					allPrefs.Add(manualRoute.Pref);
					if (manualRoute.IncludeSubcodes)
						includeSubCodesPref.Add(manualRoute.Pref);
				}
				manualRouteIterator = new RoutePrefIterator();
				manualRouteIterator.ExactIterator = new RouteIterator(allPrefs);
				manualRouteIterator.LongestMatchIterator = new RouteIterator(includeSubCodesPref);
			}
			#endregion
				...
				if (CarrierMappings.TryGetValue(route.CustomerId, out carrierMapping))
				{
					...
					foreach (string customerMapping in carrierMapping.CustomerMapping)
					{
						if (string.IsNullOrEmpty(customerMapping))
							continue;
						var pref = string.Format("{0}{1}", customerMapping, route.Code);

						//check if there is manual route for this pref 
						if (manualRouteIterator != null && !string.IsNullOrEmpty(manualRouteIterator.GetMatched(pref)))
							continue;

						if (concatenatedOptions == null)
							concatenatedOptions = BuildOptions(route, supplierOptionsSeparator);
		...
		{
			#region apply manual routes to DB
			if (ManualRoutes != null && ManualRoutes.Any())
			{
				List<ConvertedRoute> manualRoutesConverted = ManualRoutes.MapRecords(item => new TelesIdbConvertedRoute() { Pref = item.Pref, Route = item.Route }).ToList<ConvertedRoute>();
				var preparedManualRoutes = this.DataManager.PrepareDataForApply(manualRoutesConverted);
				var applyContext = new SwitchRouteSynchronizerApplyRoutesContext(preparedManualRoutes, context.SwitchName, context.SwitchId, context.PreviousSwitchSyncOutput, context.WriteBusinessHandledException);
				this.DataManager.ApplySwitchRouteSyncRoutes(applyContext);
			}
			#endregion
