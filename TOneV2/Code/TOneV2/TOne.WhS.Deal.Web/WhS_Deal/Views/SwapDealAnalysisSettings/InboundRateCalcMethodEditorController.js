(function (appControllers) {
	...
	InboundRateCalcMethodEditorController.$inject = ['$scope', 'WhS_Deal_SwapDealAnalysisAPIService', 'VRNavigationService', 'UtilsService', 'VRUIUtilsService', 'VRNotificationService'];

	function InboundRateCalcMethodEditorController($scope, WhS_Deal_SwapDealAnalysisAPIService, VRNavigationService, UtilsService, VRUIUtilsService, VRNotificationService) {
		...
		function loadParameters() {
			...
			if (parameters != undefined && parameters != null) {
				...
				rateCalcMethod = parameters.inboundRateCalcMethod;
			}
		...
		function setTitle() {
			if (isEditMode) {
				...
				$scope.title = UtilsService.buildTitleForUpdateEditor(rateCalcMethodTitle, 'Inbound Rate Calc Method');
			}
			else
				$scope.title = UtilsService.buildTitleForAddEditor('Inbound Rate Calc Method');
		}
		function loadRateCalcMethodSelector() {
			return WhS_Deal_SwapDealAnalysisAPIService.GetInboundRateCalcMethodExtensionConfigs().then(function (response) {
				if (response != null) {
		...
		function insertRateCalcMethod() {
			...
			if ($scope.onInboundRateCalcMethodAdded != undefined)
				$scope.onInboundRateCalcMethodAdded(rateCalcMethod);
			$scope.modalContext.closeModal();
		}
		function updateRateCalcMethod() {
			var rateCalcMethod = buildRateCalcMethodFromScope();
			if ($scope.onInboundRateCalcMethodUpdated != undefined)
				$scope.onInboundRateCalcMethodUpdated(rateCalcMethod);
			$scope.modalContext.closeModal();
		}
	...
	appControllers.controller('WhS_Deal_InboundRateCalcMethodEditorController', InboundRateCalcMethodEditorController);
