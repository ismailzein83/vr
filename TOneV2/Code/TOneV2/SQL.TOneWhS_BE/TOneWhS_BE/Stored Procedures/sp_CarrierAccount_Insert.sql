...
BEGIN
	set @id = 0;
	if not exists(select 1 from TOneWhS_BE.CarrierAccount where (@Name is null or [NameSuffix] = @Name ) and [CarrierProfileID] = @CarrierProfileId and ISNULL(IsDeleted,0) = 0)
	begin
