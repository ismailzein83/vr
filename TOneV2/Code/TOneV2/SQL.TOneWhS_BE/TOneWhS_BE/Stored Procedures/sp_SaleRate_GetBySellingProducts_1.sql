...
BEGIN
	...
	where	(rate.EED is null or rate.EED > rate.BED) and pricelist.OwnerType = 0 
			and pricelist.OwnerID in (select SellingProductId from @SellingProductIdsTable) and rate.ZoneID in (select SaleZoneId from @SaleZoneIdsTable)
END