...
AS
BEGIN
IF NOT EXISTS(select 1 from TOneWhS_BE.CarrierAccount WHERE  isnull([NameSuffix],'') = isnull(@Name,'') and [CarrierProfileID] = @CarrierProfileId and Id!=@ID and ISNULL(IsDeleted,0) = 0) 
BEGIN
