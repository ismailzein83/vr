...
create PROCEDURE [dbo].[SP_TrafficStats_ZoneSuppliers_new]
   @fromDate datetime,
   @ToDate datetime,
...
SELECT 
    ...
    WHERE (FirstCDRAttempt >= @FromDate AND FirstCDRAttempt <= @ToDate)
                  AND (TS.CustomerID = @CustomerID) AND ts.CustomerID NOT IN ( SELECT grasc.CID
...
GROUP BY bs.Cost_Currency,bs.Sale_Currency,BS.SupplierID,bs.SaleZoneID
)
