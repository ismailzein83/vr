...
CREATE PROCEDURE [dbo].[SP_TrafficStats_CustomersByZone]
   @fromDate datetime,
   @ToDate datetime,
   @ZoneID INT 
AS
...
INSERT INTO @ExchangeRates SELECT * FROM dbo.GetDailyExchangeRates(@FromDate, dateadd(d,-1,@ToDate))

;With 
Traffic AS
	(
		SELECT 
		   ISNULL(TS.CustomerID, '') CustomerID,
...
SELECT	CustomerID ,
		Attempts , 
		CASE WHEN TotalCTE.TotalAttempts <>0 THEN  (Attempts * 100. / TotalCTE.TotalAttempts) ELSE 0.0 END AttemptPercentage , 
		DurationsInMinutes , 
		SuccessfulAttempts ,
		ASR , 
		ACD , 
