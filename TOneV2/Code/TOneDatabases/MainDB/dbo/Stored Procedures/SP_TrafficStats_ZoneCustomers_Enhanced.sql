...
CREATE PROCEDURE [dbo].[SP_TrafficStats_ZoneCustomers_Enhanced]
   @fromDate datetime,
   @ToDate datetime,
...
INSERT INTO @Result(CustomerID ,Attempts, DurationsInMinutes,SuccessfulAttempts, ASR, ACD, DeliveredASR, AveragePDD, NumberOfCalls, Cost_Nets, Sale_Nets, Profit, Percentage)
    ...
    FROM @Traffic TS 
		...
		And (BS.CallDate >= @fromDate AND BS.CallDate <= @ToDate) AND (BS.SupplierID=@SupplierID) AND BS.CostZoneID = @ZoneID
