...
CREATE    PROCEDURE [dbo].[SP_TrafficStats_SupplierSummary_Enhanced]
   @fromDate datetime ,
   @ToDate datetime,
...
;WITH Traffic_ AS
	(
		...
		FROM
			TrafficStats ts  WITH(NOLOCK,INDEX(IX_TrafficStats_DateTimeFirst))
...
(
	SELECT
			  ...
			  ISNULL(SUM(BS.Sale_Nets/ISNULL(ERS.Rate, 1)),0)-ISNULL(SUM(BS.Cost_Nets/ISNULL(ERS.Rate, 1)),0) AS Profit,
