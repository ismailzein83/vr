...
BEGIN
	...
	Declare @Results TABLE ( AttemptDateTime DATETIME, OurZoneID INT ,Attempts int, FailedAttempts int, DurationsInMinutes numeric(13,5), CeiledDuration numeric(13,5), ASR numeric(13,5), ACD numeric(13,5), DeliveredAttempts INT, DeliveredASR numeric(13,5), AveragePDD numeric(13,5), MaxDuration numeric(13,5), LastAttempt datetime, AttemptPercentage numeric(13,5),DurationPercentage numeric(13,5),SuccesfulAttempts int, ZoneName nvarchar(250) )
	SET NOCOUNT ON
	-- Customer, No Supplier
	IF @CustomerID IS NOT NULL AND @SupplierID IS NULL
		INSERT INTO @Results (AttemptDateTime ,OurZoneID,Attempts, DurationsInMinutes, CeiledDuration ,ASR,ACD, deliveredAttempts,DeliveredASR,AveragePDD,MaxDuration,LastAttempt,AttemptPercentage,DurationPercentage,SuccesfulAttempts, FailedAttempts, ZoneName)
			SELECT 
