...
CREATE PROCEDURE [dbo].[SP_TrafficStats_CustomerSummary_Enhanced]
   @fromDate datetime ,
   @ToDate datetime,
...
(
	SELECT
			  ...
			  Sum(bs.CostDuration/60.) as DurationsInMinutes, 
...
, Results AS 
(
	SELECT  T.CustomerID, T.Attempts,isnull(T.DurationsInMinutes,0) AS DurationsInMinutes, T.ASR, T.ACD, T.DeliveredASR, T.AveragePDD,
	        B.Calls AS NumberOfCalls, isnull(B.Sale,0) AS Sale_Nets,isnull(B.Cost,0) AS Cost_Nets,isnull(B.Profit,0) AS Profit,0 AS Percentage
	         ,  ROW_NUMBER() OVER (ORDER BY T.DurationsInMinutes DESC) AS rownIndex
