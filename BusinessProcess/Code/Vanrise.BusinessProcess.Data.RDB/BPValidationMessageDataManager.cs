...
//        }
//        public IEnumerable<BPValidationMessage> GetFilteredBPValidationMessage(DataRetrievalInput<BPValidationMessageQuery> input)
//        {
//            var queryContext = new RDBQueryContext(GetDataProvider());
...
//            var whereContext = selectQuery.Where();
//            if (input.Query.Severities != null && input.Query.Severities.Count > 0)
//                whereContext.ListCondition(COL_Severity, RDBListConditionOperator.IN, input.Query.Severities.Select(itm => (int)itm));
//            whereContext.EqualsCondition(COL_ProcessInstanceID).Value(input.Query.ProcessInstanceId);
