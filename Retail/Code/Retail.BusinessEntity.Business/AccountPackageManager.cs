    ...
    public class AccountPackageManager
    {
        ...
        public IDataRetrievalResult<AccountPackageDetail> GetFilteredAccountPackages(DataRetrievalInput<AccountPackageQuery> input)
        {
            ...
            Func<AccountPackage, bool> filterExpression = (accountPackage) =>
            {
                ...
                if (input.Query.CurrencyIds != null && (!context.CurrencyId.HasValue || !input.Query.CurrencyIds.Contains(context.CurrencyId.Value)))
                    return false;

                if (input.Query.BED.HasValue && input.Query.BED.VRGreaterThan(accountPackage.BED))
                    return false;
        ...
        private Dictionary<long, AccountPackage> GetCachedAccountPackages()
        {
        ...
        private bool IsPackageAssignmentValid(long accountPackageId, int packageId, long accountId, DateTime bed, DateTime? eed, out Package package, out Account account, out Guid accountBEDefinitionId, out string errorMessage)
        {
        ...
        private class AccountPackageExcelExportHandler : ExcelExportHandler<AccountPackageDetail>
        {
            public override void ConvertResultToExcelData(IConvertResultToExcelDataContext<AccountPackageDetail> context)
            {
                ...
                sheet.Header.Cells.Add(new ExportExcelHeaderCell { Title = "Account", Width = 20 });
                sheet.Header.Cells.Add(new ExportExcelHeaderCell { Title = "Package", Width = 20 });
                sheet.Header.Cells.Add(new ExportExcelHeaderCell { Title = "Package Type", Width = 20 });
                sheet.Header.Cells.Add(new ExportExcelHeaderCell { Title = "Status", Width = 20 });
                sheet.Header.Cells.Add(new ExportExcelHeaderCell { Title = "Currency", Width = 20 });
                sheet.Header.Cells.Add(new ExportExcelHeaderCell { Title = "Charge Value", Width = 20 });
                sheet.Header.Cells.Add(new ExportExcelHeaderCell { Title = "Period Type", Width = 20 });
                ...
                if (context.BigResult != null && context.BigResult.Data != null)
                {
                    foreach (var record in context.BigResult.Data)
                    {
                        if (record.Entity != null)
                        {
                            ...
                            row.Cells.Add(new ExportExcelCell { Value = record.PackageType });
                            row.Cells.Add(new ExportExcelCell { Value = record.StatusName });
                            row.Cells.Add(new ExportExcelCell { Value = record.CurrencySymbol });
                            row.Cells.Add(new ExportExcelCell { Value = record.ChargeValue });
                            row.Cells.Add(new ExportExcelCell { Value = record.PeriodType });
        ...
        private class AccountInfo
        {
            ...
            public List<AccountPackage> AccountPackages { get { return _accountPackages; } }
        ...
        AccountPackageDetail AccountPackageDetailMapper(AccountPackage accountPackage)
        {
            ...
            return new AccountPackageDetail()
            {
                ...
                CurrencySymbol = (context.CurrencyId.HasValue) ? currencyManager.GetCurrencySymbol(context.CurrencyId.Value) : String.Empty,
                PeriodType = context.PeriodType,
                PackageType = packageDefinition.GetPackageDefinitionName(package.Settings.PackageDefinitionId)
            };
        }
