    ...
    internal class GenericAnalyticConfigManager
    {
        ...
        private static void FillDimensionsConfig()
        {
            ...
            s_AllDimensionsConfig.Add(AnalyticDimension.Zone,
               ...
               {
                   IdColumn = "ISNULL(ts.ZoneID,'N/A')",
                   NameColumn = "salz.Name",
                   JoinStatements = new List<string>() { " JOIN dbo.Zone salz WITH (NOLOCK) ON salz.ID = ts.ZoneID " },
                   GroupByStatements = new List<string>() { " ts.ZoneID, salz.Name " },
            ...
            #region OperatorPart
            s_AllDimensionsConfig.Add(AnalyticDimension.Operator,
             new AnalyticDimensionConfig
             {
                 IdColumn = "ISNULL(ts.OperatorID,0)",
                 NameColumn = "ISNULL(case when op.NameSuffix != '' THEN  opProf.Name + ' (' + op.NameSuffix + ')' else opProf.Name end,'N/A')",
                 JoinStatements = new List<string>() { @"LEFT JOIN dbo.OperatorAccount op WITH (NOLOCK) ON op.ID = ts.OperatorID 
                                                            LEFT JOIN dbo.OperatorProfile opProf on op.OperatorProfileID = opProf.ID " },
                 GroupByStatements = new List<string>() { " ts.OperatorID, op.NameSuffix, opProf.Name " },
                 ExpressionSummary = AnalyticSummary.Sum.ToString("G")
             });
            #endregion
            ...
            s_AllDimensionsConfig.Add(AnalyticDimension.Country,
                new AnalyticDimensionConfig
                {
                    ...
                    NameColumn = "c.Name",
                    JoinStatements = new List<string>() { @" JOIN  dbo.Zone ourz ON ts.ZoneID = ourz.ID JOIN [dbo].Country c ON ourz.CountryID = c.ID" },
            ...
            #region ServiceTypePart
            s_AllDimensionsConfig.Add(AnalyticDimension.ServiceType,
               new AnalyticDimensionConfig
               {
              ...
              new AnalyticMeasureFieldConfig
              {
                  GetColumnsExpressions = new List<Func<GenericAnalyticQuery, MeasureValueExpression>>{
                        (query) =>
                       {
                           ...
                           bool filterOperator = false;
                           foreach (DimensionFilter dimensionFilter in query.Filters)
                           {
                               if (dimensionFilter.Dimension == AnalyticDimension.Operator)
                                   filterOperator = true;
                           }
               ...
               new AnalyticMeasureFieldConfig
               {
                   GetColumnsExpressions = new List<Func<GenericAnalyticQuery, MeasureValueExpression>>{
                        (query) =>
                       {
                           ...
                           return new MeasureValueExpression
                           {
                               Expression = String.Format("Case WHEN (Sum(ts.{0})-Sum(case when ts.OperatorID is null then ts.Attempts else 0 end))>0 THEN CONVERT(DECIMAL(10,2),SUM(ts.DeliveredNumberOfCalls)*100.0/(Sum(ts.{0})-Sum(case when ts.OperatorID is null then ts.Attempts else 0 end))) ELSE 0 END ", s),
