...
namespace Vanrise.Fzero.FraudAnalysis.BP.Arguments
{
    public class ExecuteStrategyForNumberRangeProcessInput : Vanrise.BusinessProcess.Entities.BaseProcessInputArgument
    {
        ...
        public override string GetTitle()
        {
            ...
            return String.Format("Execute Strategy Process For Number Prefix '{0}', Time Range ({1:dd-MMM-yy HH:00} to {2:dd-MMM-yy HH:00}), Strategies: {3}", this.NumberPrefix, this.FromDate, this.ToDate, String.Join(",", strategyManager.GetStrategyNames(ExecuteStrategiesExecutionItems.Select(itm => itm.StrategyId).ToList())));
        }
    }
