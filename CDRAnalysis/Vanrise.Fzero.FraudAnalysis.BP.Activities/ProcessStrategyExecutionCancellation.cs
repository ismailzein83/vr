        ...
        protected override void DoWork(ProcessStrategyExecutionCancellationInput inputArgument, AsyncActivityHandle handle)
        {
            ...
            foreach (StrategyExecutionItem item in items)
            {
                ...
                if (item.CaseID != null)
                {
                    ...
                    {
                        if (!itemsofCases.Exists(x => x.CaseID == relatedCase.CaseID && x.StrategyExecutionID != inputArgument.StrategyExecutionId && x.SuspicionOccuranceStatus != SuspicionOccuranceStatus.Deleted))
                        {
