...
CREATE PROCEDURE [FraudAnalysis].[sp_GetSuspiciousNumbers]
	(
	@fromDate datetime,
	@ToDate DATETIME,
	@strategyId INT,
...
,'' 
from [FraudAnalysis].[SubscriberThresholds] st
...
group by SubscriberNumber,SuspectionLevelId,sl.[Name] 
 ...
 FROM #temp t
 SELECT @strategyId as strategyId, t.*, s.Name as StrategyName FROM #temp t inner join [FraudAnalysis].Strategy s on s.Id=@strategyId
