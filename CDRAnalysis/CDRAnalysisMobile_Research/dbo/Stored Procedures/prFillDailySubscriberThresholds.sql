CREATE PROCEDURE [dbo].[prFillDailySubscriberThresholds] 
(
   @StrategyId int 
) 
AS
...
--========== setting Control Table    =================
 ...
 SELECT @minValue = Min_count_value FROM  Strategy_Min_Values
  WHERE strategyid = @StrategyId  AND    criteriaid = 9 
...
set @CtrlTableId=@@IDENTITY
 ...
 if @StartingDate is null
  begin
     SELECT  @StartingDate= CONVERT(datetime,CONVERT(char,min(FromDate),106))from dbo.NumberProfile WHERE PeriodId=1
  END
     SELECT  @EndingDate= CONVERT(datetime,CONVERT(char,MAX(FromDate),106))from dbo.NumberProfile WHERE PeriodId=1
...
--=============================================  
 ...
 INSERT INTO #temp
  ...
  ---===================== Criteria9 ========  
   INSERT INTO #temp
    (DateDay, SubscriberNumber, Criteria9, SuspectionLevelId, StrategyId, Periodid)
  SELECT
  CONVERT(datetime,CONVERT(char,sv.FromDate,106)),
  sv.SubscriberNumber,COUNT(DISTINCT  DATEPART(hour,sv.FromDate))*1.00 / @ThCr9 * 1.00,
    1,@StrategyId,1
  FROM
    NumberProfile sv,Strategy_Min_Values v
  WHERE
    sv.FromDate >= @StartingDate AND
    sv.FromDate <= @EndingDate AND
    sv.PeriodId = 6 AND
    sv.Count_out_Calls > @minValue AND
    @prCr9 = 1
  GROUP BY
    sv.SubscriberNumber,
    sv.PeriodId,
  ...
  INSERT INTO #temp
    (DateDay, SubscriberNumber, Criteria14, SuspectionLevelId, StrategyId, Periodid)
  SELECT
    CONVERT(datetime,CONVERT(char,sv.FromDate,106)),
    sv.SubscriberNumber,
    SUM(sv.Count_out_Calls)*1.00 / @ThCr14*1.00,
    1,
    @StrategyId,
    1
  FROM
  ...
  INSERT INTO #temp
    (DateDay, SubscriberNumber, Criteria10, SuspectionLevelId, StrategyId, Periodid)

  SELECT
    CONVERT(datetime,CONVERT(char,nc.connectDateTime,106)),
    MSISDN SubscriberNumber,
    (COUNT(DISTINCT (CASE WHEN Call_Type = 1 THEN destination END)) * 1.00) / (@ThCr10 * 1.00)
    ,
    1,
    @StrategyId,
    1
  ...
  WHERE
    (
    	 (
    	 	nc.connectDateTime >=(SELECT CONVERT(datetime,CONVERT(char,@StartingDate,106)))
            AND   nc.connectDateTime < (SELECT  CONVERT(datetime,CONVERT(char,@EndingDate,106))) 
            AND    DATEPART(hour,nc.connectDateTime) IN  (0, 1, 2, 3, 4, 5)
    	 )
    	  OR
         (
            nc.connectDateTime >=  (SELECT CONVERT(datetime,CONVERT(char,DATEADD(DAY,1,@StartingDate),106)))
            AND nc.connectDateTime <(SELECT CONVERT(datetime,CONVERT(char,DATEADD(DAY,1,@EndingDate),106)))
...
CREATE TABLE #tempNew(	[DateDay] [datetime] ,	[SubscriberNumber] [varchar](50) ,
	[Criteria1] decimal(18,2) ,	[Criteria2] decimal(18,2) ,	[Criteria3] decimal(18,2) ,
	[Criteria4] decimal(18,2) ,	[Criteria5] decimal(18,2) ,	[Criteria6] decimal(18,2) ,	
	[Criteria7] decimal(18,2) ,	[Criteria8] decimal(18,2) ,[Criteria9] decimal(18,2) ,
	[Criteria10] decimal(18,2),[Criteria11] decimal(18,2) ,[Criteria12] decimal(18,2) ,
...
-- ==============
 ...
 ts_prFillDailySubscriberThresholds 28
