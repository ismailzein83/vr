CREATE PROCEDURE [dbo].[Ol_prFillHourlySubscriberThresholds]
(
   @StrategyId int 
)
...
set @CtrlTableId=@@IDENTITY
 ...
 begin
  select @startingId= min(sv.id) from dbo.Subscriber_Values sv  WHERE sv.PeriodId=6
 end
 if @StartingDate is null
  begin
     SELECT  @StartingDate= DATEADD(hour, DATEDIFF(hour, 0, min(FromDate)), 0)from dbo.Subscriber_Values WHERE PeriodId=6
  end
     SELECT  @EndingDate= DATEADD(hour, DATEDIFF(hour, 0, MAX(FromDate)), 0)from dbo.Subscriber_Values  WHERE PeriodId=6
...
--=============================================
			INSERT INTO #temp
				...
				,case when sv.CriteriaId = 1 AND sv.PeriodId= 6 and @prCr1=6  then sv.VALUE*1.00/@ThCr1*1.00 else null end Criteria1
				,case when sv.CriteriaId = 2  AND sv.PeriodId= 6 and @prCr2=6 then sv.VALUE*1.00/@ThCr2*1.00 else null end Criteria2
				,case when sv.CriteriaId = 3  AND sv.PeriodId= 6 and @prCr3=6 then sv.VALUE*1.00/@ThCr3*1.00 else null end Criteria3
				,case when sv.CriteriaId = 4  AND sv.PeriodId= 6 and @prCr4=6 then sv.VALUE*1.00/@ThCr4*1.00 else null end Criteria4
				,case when sv.CriteriaId = 5  AND sv.PeriodId= 6 and @prCr5=6 then sv.VALUE*1.00/@ThCr5*1.00 else null end Criteria5
				
				,case when sv.CriteriaId = 6  AND sv.PeriodId= 6 and @prCr6=6 then sv.VALUE*1.00/@ThCr6*1.00 else null end Criteria6
				,case when sv.CriteriaId = 7  AND sv.PeriodId= 6 and @prCr7=6 then sv.VALUE*1.00/@ThCr7*1.00 else null end Criteria7
				,case when sv.CriteriaId = 8  AND sv.PeriodId= 6 and @prCr8=6 then sv.VALUE*1.00/@ThCr8*1.00 else null end Criteria8
				,case when sv.CriteriaId = 9  AND sv.PeriodId= 6 and @prCr9=6 then sv.VALUE*1.00/@ThCr9*1.00 else null end Criteria9
				,case when sv.CriteriaId = 10  AND sv.PeriodId= 6 and @prCr10=6 then sv.VALUE*1.00/@ThCr10*1.00 else null end Criteria10
				,case when sv.CriteriaId = 11  AND sv.PeriodId= 6 and @prCr11=6 then sv.VALUE*1.00/@ThCr11*1.00 else null end Criteria11
				,case when sv.CriteriaId = 12  AND sv.PeriodId= 6 and @prCr12=6 then sv.VALUE*1.00/@ThCr12*1.00 else null end Criteria12
				,case when sv.CriteriaId = 13  AND sv.PeriodId= 6 and @prCr13=6 then sv.VALUE*1.00/@ThCr13*1.00 else null end Criteria13
				,case when sv.CriteriaId = 14  AND sv.PeriodId= 6 and @prCr14=6 then sv.VALUE*1.00/@ThCr14*1.00 else null end Criteria14
				,case when sv.CriteriaId = 15  AND sv.PeriodId= 6 and @prCr15=6 then sv.VALUE*1.00/@ThCr15*1.00 else null end Criteria15
...
where st.StrategyId=@StrategyId and st.DateDay between @StartingDate and @EndingDate AND st.Periodid=6
...
/*
 ...
 [prFillHourlySubscriberThresholdstest]
     @StrategyId =1
...
SELECT * FROM SubscriberThresholds st WHERE st.SuspectionLevelId IS NOT null

select * from SubscriberThresholds st WHERE st.SuspectionLevelId=2
*/