CREATE PROCEDURE [dbo].[Ol_prFillDailyProfile]
AS
BEGIN
...
-------------------------- Served Party Profile     ---------------------- 
     ...
     insert into dbo.Ol_NumberProfile
	...
	--=====
	group by MSISDN , convert(varchar(12), connectDateTime , 102 )  ,  DATEPART(day, connectdatetime )
	...
	---- No Data Available  ----,COUNT(distinct(case when  Call_Type=1 then  BTS_id  end )) total_BTS_In
	---- No Data Available  ----,COUNT(distinct(case when  Call_Type=2 then  BTS_id  end )) total_BTS_Out
	...
	--=====
	group by Destination , convert(varchar(12), connectDateTime , 102 )  ,  DATEPART(day, connectdatetime )
...
/*
 ...
 [prFillDailyProfile]
 --truncate table Ol_NumberProfile
 select * from Ol_NumberProfile
