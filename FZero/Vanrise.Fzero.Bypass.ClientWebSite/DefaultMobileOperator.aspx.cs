...
public partial class DefaultMobileOperator : MobileOperatorPage
{
    #region Methods
    ...
    private void FillCombos()
    {
        ...
        ddlMobileOperatorFeedback.Items.FindItemByValue(((int)Vanrise.Fzero.Bypass.Enums.MobileOperatorFeedbacks.Pending).ToString()).Visible = false;
    }
    ...
    private void Populate()
    {
        ...
        {
            case 2:
                List<Vanrise.Fzero.Bypass.prGetTimeActiveonNetwork_Result> lstView_TimeActiveonNetwork = Vanrise.Fzero.Bypass.prGetTimeActiveonNetwork_Result.prGetTimeActiveonNetwork(ConfigurationManager.AppSettings["ClientID"].ToInt(), CurrentUser.MobileOperatorID, fromDateTime, toDateTime);
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            int DifferenceinGMT = CurrentUser.User.GMT - SysParameter.Global_GMT;
            rdpFrom.SelectedDate = DateTime.Now.AddHours(DifferenceinGMT).Date;
            rdpTo.SelectedDate = DateTime.Now.AddHours(DifferenceinGMT).Date.AddDays(1).AddSeconds(-1);
            Populate();
            FillCombos();
    ...
    protected void btnConfirm_Click(object sender, EventArgs e)
    {
        ...
        Vanrise.Fzero.Bypass.GeneratedCall.Feedback(ListIds, ddlMobileOperatorFeedback.SelectedValue.ToInt(), txtFeedbackNotes.Text, DateTime.Now);
        gvGeneratedCalls.Rebind();
        Vanrise.Fzero.Bypass.LoggedAction.AddLoggedAction((int)Vanrise.Fzero.Bypass.Enums.ActionTypes.Repliedonreportedcases, CurrentUser.User.ID);
        Vanrise.Fzero.Bypass.EmailManager.SendMobileOperatorFeedback(Vanrise.Fzero.Bypass.SysParameter.Global_SenderEmail, ListCaseIds, ReportRealID, "FMS_Profile");
