...
namespace Vanrise.Fzero.Services.RecievedMysql
{
    public partial class RecievedMysqlService : ServiceBase
    {
        private static System.Timers.Timer aTimer;

        public RecievedMysqlService()
        {
            try
            {
                InitializeComponent();
            ...
            {
                ErrorLog("RecievedMysqlService() " + ex.Message);
                ErrorLog("RecievedMysqlService() " + ex.ToString());
                ErrorLog("RecievedMysqlService() " + ex.InnerException.ToString());
            }
                ...
                if (HttpHelper.CheckInternetConnection("smtp.gmail.com", 587))
                {
                    ...
                    if (listcalls[0].Count() > 0 || listcallsfailed[0].Count() > 0)
                    {
                        ...
                        for (int i = 0; i < listcalls[0].Count(); i++)
                        {
                            ...
                            elements.Add(new XElement("ClientName", listcalls[6][i]));
                        ...
                        for (int i = 0; i < listcallsfailed[0].Count(); i++)
                        {
                            ...
                            elements.Add(new XElement("ClientName", listcallsfailed[6][i]));
                            cdr = new XElement("CDR", elements);
                            cdrs.Add(cdr);
                        }
                        ...
                        db.SendMail(System.Configuration.ConfigurationManager.AppSettings["ToAddress"].ToString(), "Recieved Mysql Calls", "Administrator", "Please find attached XML of Calls", Path, string.Empty, null);
                    }
                }
    ...
    class DBConnect
    {
        ...
        public List<string>[] SelectCalls(int LastCallID)
        {
            try
            {
                ...
                string querycalls = "SELECT   a.id_call as ID,  IFNULL(c.Name,'ITPC') as ClientName,  a.id_call, a.caller_id, a.called_number, a.call_start, a.duration  FROM    calls as a left outer join clients as c on    a.id_Client = c.ID where id_call>" + LastCallID.ToString();

                //Create a list to store the result
                List<string>[] listcalls = new List<string>[9];
                ...
                if (this.OpenConnection() == true)
                {
                    ...
                    while (dataReader.Read())
                    {
                        ...
                        listcalls[6].Add(dataReader["ClientName"] + "");
                    }
            ...
            {
                ErrorLog("SelectCalls() " + ex.Message);
                ErrorLog("SelectCalls() " + ex.ToString());
                ErrorLog("SelectCalls() " + ex.InnerException.ToString());
        ...
        public List<string>[] SelectCallsFailed(int LastCallFailedID)
        {
            try
            {
                ...
                string querycallsfailed = "SELECT     a.id_failed_call as ID, IFNULL(c.Name,'ITPC') as ClientName, a.id_failed_call, a.caller_id, a.called_number , a.call_start, 0 as duration   FROM         callsfailed as a  left outer join clients as c on    a.id_Client = c.ID  where id_failed_call>" + LastCallFailedID.ToString();

                //Create a list to store the result
                List<string>[] list_failedcalls = new List<string>[9];
                ...
                if (this.OpenConnection() == true)
                {
                    ...
                    while (dataReaderfailed.Read())
                    {
                        ...
                        list_failedcalls[6].Add(dataReaderfailed["ClientName"] + "");
                    }
            ...
            {
                ErrorLog("SelectCallsFailed() " + ex.Message);
                ErrorLog("SelectCallsFailed() " + ex.ToString());
                ErrorLog("SelectCallsFailed() " + ex.InnerException.ToString());
                return new List<string>[9];
            }
              ...
              try
                {
                  ...
                  {
                      ErrorLog("LastCallID() " + ex.Message);
                      ErrorLog("LastCallID() " + ex.ToString());
                      ErrorLog("LastCallID() " + ex.InnerException.ToString());
                      return 0;
                  }
        ...
        public int LastCallFailedID()
        {
            ...
            {
                ErrorLog("LastCallFailedID() " + ex.Message);
                ErrorLog("LastCallFailedID() " + ex.ToString());
                ErrorLog("LastCallFailedID() " + ex.InnerException.ToString());
                return 0;
            }
