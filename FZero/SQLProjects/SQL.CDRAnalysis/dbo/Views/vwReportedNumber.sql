...
CREATE VIEW [dbo].[vwReportedNumber]
AS
SELECT    top 10000000 dbo.NumberProfile.SubscriberNumber as CLI, dbo.NumberProfile.Date_Day, (CASE WHEN Total_Out_Volume = - 1 THEN 0 ELSE Total_Out_Volume END) AS OriginatedVolume, 
                      (CASE WHEN Total_In_Volume = - 1 THEN 0 ELSE Total_In_Volume END) AS TerminatedVolume, (CASE WHEN Count_Out_Calls = - 1 THEN 0 ELSE Count_Out_Calls END) 
                      AS NumberofAttempts, (CASE WHEN Diff_Output_Numb = - 1 THEN 0 ELSE Diff_Output_Numb END) AS CountDistinctCalledParties, dbo.Report.Id AS ReportID, 
...
GO
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPaneCount', @value = 1, @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'vwReportedNumber';
...
Begin DesignProperties = 
   ...
   End
End
', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'vwReportedNumber';
