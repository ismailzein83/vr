...
FROM SourceMappings s inner join PredefinedColumns p on  s.MappedtoColumnNumber = p.ID
...
set @Query='insert into CDR (SourceID, '+@AllInsertColumnNames+', ImportID) select  top 100000 '+ cast( @SwitchID as varchar(4))+' as SourceID, '+@AllSelectColumnNames+', '+ cast( @ImportID as varchar(4)) +'  from ['+@DBName+'].dbo.CDR Where ID >'+cast( @Reference as varchar(100))+''
print @Query
exec (@Query)
