...
update GeneratedCalls set reportingstatusID=3 where ID in (SELECT gc.ID FROM  RecievedCalls rc  inner JOIN dbo.GeneratedCalls gc ON gc.ID = rc.GeneratedCallID  where  reportingstatusID=1 and     (rc.CLI IN   (SELECT     CLI   FROM    vwCLIs where CLI is not null)) )
update GeneratedCalls set  LevelOneComparisonDateTime=@LevelOneComparisonDateTime, Level1Comparison =1 where SourceID=5 and Level1Comparison=0
update GeneratedCalls set statusID=3, PriorityID=1, CLI='Private Number'  where ID in (select gc.Id from GeneratedCalls gc inner join RecievedCalls rc  on gc.ID=rc.GeneratedCallID where   gc.b_number=rc.cli)
...
DECLARE @AttemptDateTime datetime
 ...
 SELECT  [ID] ,[a_number] ,[b_number] ,[AttemptDateTime] FROM GeneratedCalls where Level1Comparison=0 and Level2Comparison=0 and StatusID=1  and attemptdatetime >DATEADD(dd, -1, GETDATE()) 
OPEN @getID
FETCH NEXT
FROM @getID INTO @ID  ,@a_number ,@b_number ,@AttemptDateTime
...
BEGIN
 ...
 select top 1 ID,  CLI, a_number, b_number, AttemptDateTime, dbo.fn_CheckIfEqual(CLI,b_number) as IsEqual ,  dbo.fn_CheckIfLocal(CLI) as IsLocal, ABS (DATEDIFF(SECOND, AttemptDateTime, @AttemptDateTime))as DateDifference from RecievedCalls where 
    ( GeneratedCallID=@ID  )
    ;
  ...
  if ((select COUNT(*) from #RecievedCalls) = 0)
   Begin
    INSERT INTO #RecievedCalls (ID,  CLI, a_number, b_number, AttemptDateTime, IsEqual, IsLocal, DateDifference ) 
 select top 1 ID,  CLI, a_number, b_number, AttemptDateTime, dbo.fn_CheckIfEqual(CLI,b_number) as IsEqual ,  dbo.fn_CheckIfLocal(CLI) as IsLocal, ABS (DATEDIFF(SECOND, AttemptDateTime, @AttemptDateTime))as DateDifference from RecievedCalls where 
 (b_number =@RecievedB_number 
  or b_number =@RecievedB_number_without_964 
  or b_number =@RecievedB_number_without_963 
  ) 
    
    and (GeneratedCallID is NULL )
    and (AttemptDateTime >= @RecievedFromAttemptDateTime  and AttemptDateTime <= @RecievedToAttemptDateTime) 
   ...
   if ((select COUNT(*) from #RecievedCalls) >0)
   Begin
   set @IsEqual = (select top 1 IsEqual from #RecievedCalls);
   set @IsLocal = (select top 1 IsLocal from #RecievedCalls);
  
   
     Declare @CLI varchar(10)
     set @CLI =(select top 1 CLI from #RecievedCalls);

 

	 set @HasEmptyCLI =(SELECT CASE  WHEN (@CLI IS  NULL) THEN 1   WHEN (@CLI ='') THEN 1   ELSE 0 END );
   
     update RecievedCalls set GeneratedCallID=@ID where ID= (select top 1 ID from #RecievedCalls);
       ...
       if (@HasEmptyCLI=1)
		  ...
		  -- if ((select (case when @b_number like '964%' then  1 when @b_number like '963%' then  1  else 0 end) IsTermination )=1)
		   
				--Begin
				--  update GeneratedCalls set StatusID=2, LevelOneComparisonDateTime=@LevelOneComparisonDateTime, Level1Comparison =1 where ID = @ID 
				--End
		   ...
		   Begin
			  update GeneratedCalls set StatusID=4, LevelOneComparisonDateTime=@LevelOneComparisonDateTime, Level1Comparison =1 where  ID = @ID 
		   End
      
   End
    else if (@AttemptDateTime <= DATEADD(DD, -4 ,@LevelOneComparisonDateTime) )
		   Begin
			update  GeneratedCalls set Level1Comparison=1 , StatusID=5, LevelOneComparisonDateTime=@LevelOneComparisonDateTime where  ID = @ID 
		   End
