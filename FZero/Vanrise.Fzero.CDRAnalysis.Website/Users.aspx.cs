...
public partial class Users : BasePage
{
    ...
    public User SetData()
    {
        ...
        Vanrise.Fzero.CDRAnalysis.User User = currentObject;
    ...
    private bool Save()
    {
        ...
        if (currentObject.ID == 0)
        {
            ...
            if (Vanrise.Fzero.CDRAnalysis.User.IsUserNameExists(username))
            {
        ...
        User.Save(User);
        List<Vanrise.Fzero.CDRAnalysis.UserPermission> userPermissions = SetAdminPermissions(User.ID, CurrentUser.User.ID);
        Vanrise.Fzero.CDRAnalysis.UserPermission.DeleteByUserId(User.ID);
        Vanrise.Fzero.CDRAnalysis.UserPermission.Save(userPermissions);
    ...
    protected void gvApplicationUsers_NeedDataSource(object source, Telerik.Web.UI.GridNeedDataSourceEventArgs e)
    {
        ...
        gvApplicationUsers.DataSource = Vanrise.Fzero.CDRAnalysis.User.GetUsers(UserName, FullName, EmailAddress, Address, MobileNumber, IsActive);
    ...
    protected void gvApplicationUsers_ItemCommand(object sender, GridCommandEventArgs e)
    {
        ...
        {
            case "Remove":
                if (Vanrise.Fzero.CDRAnalysis.User.Delete(Id))
                {
                    gvApplicationUsers.Rebind();
                    ClearForm();
                    ClearSearchForm();
            ...
            case "Modify":
                currentObject = Vanrise.Fzero.CDRAnalysis.User.Load(Id);
            ...
            case "ActivateDeactivate":
                if (Vanrise.Fzero.CDRAnalysis. User.ActivateDeactivate(Id))
                {
                    gvApplicationUsers.Rebind();
                    ClearSearchForm();
                }
