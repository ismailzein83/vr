...
--select m.ID as MobileOperatorID, r.ClientID,  u.FullName,  (select COUNT(*) from ViewGeneratedCalls gg
-- where gg.MobileOperatorID=m.ID and gg.ClientID=r.ClientID and  (@FromAttemptDateTime is null or  gg.AttemptDateTime>=@FromAttemptDateTime ) and ( @ToAttemptDateTime is null or   gg.AttemptDateTime<=@ToAttemptDateTime)) as Attempts
