	...
	function EndPointEditorController($scope, NP_IVSwitch_EndPointAPIService, VRNotificationService, UtilsService, VRNavigationService, VRUIUtilsService, NP_IVSwitch_StateEnum, NP_IVSwitch_RtpModeEnum, NP_IVSwitch_EndPointEnum) {
		...
		function loadParameters() {
			...
			if (parameters != undefined && parameters != null) {
				...
				if (parameters.IsClone != undefined)
					isClone = parameters.IsClone;
		...
		function defineScope() {
			$scope.scopeModel = {};
			$scope.dataItem = {};
			$scope.scopeModel.sipLoginPasswords = [];
			$scope.scopeModel.isEditMode = isEditMode;
			$scope.scopeModel.isClone = isClone;
			$scope.scopeModel.isGridValid = function () {
				if ($scope.scopeModel.sipLoginPasswords.length < 1)
					return "at least one SIP";
				return null;
			};

			$scope.scopeModel.addSipLoginPassword = function () {
				var gridItem = {
					SIPLogin: undefined,
					SIPPassword: undefined
				};
				$scope.scopeModel.sipLoginPasswords.push(gridItem);
			};

			$scope.scopeModel.removeSipLoginPassword = function (dataItem) {
				var index = $scope.scopeModel.sipLoginPasswords.indexOf(dataItem);
				$scope.scopeModel.sipLoginPasswords.splice(index, 1);
			};

			$scope.scopeModel.onSipLoginValueChanged = function (dataItem) {
				var sipLogPass = $scope.scopeModel.sipLoginPasswords;
				if (sipLogPass.length < 2) return null;
				for (var i = 0; i < sipLogPass.length; i++) {
					var item = sipLogPass[i];
					var index = sipLogPass.indexOf(item);
					var indexDataItem = sipLogPass.indexOf(dataItem);
					if (index != indexDataItem && dataItem.SIPLogin == item.SIPLogin)
						return "Recurring same SIP Login";
				}
				return null;
			};
			...
			$scope.scopeModel.onSelectionEndPointChanged = function (SelectedItem) {
				if (SelectedItem != undefined) {
					...
					if ($scope.scopeModel.endpointtype == NP_IVSwitch_EndPointEnum.SIP.value) {
		...
		function loadAllControls() {
			...
			function loadStaticData() {
				...
				if (isClone == true && endPointEntity.EndPointType == NP_IVSwitch_EndPointEnum.ACL.value) {
					$scope.scopeModel.host = "";
				}
				if (isClone == true && endPointEntity.EndPointType == NP_IVSwitch_EndPointEnum.SIP.value) {
					$scope.scopeModel.sippassword = "";
					$scope.scopeModel.siplogin = "";
				}
			}
		...
		function insert() {
			...
			return NP_IVSwitch_EndPointAPIService.AddEndPoint(buildEndPointObjFromScopeForAdd()).then(function (response) {
				if (VRNotificationService.notifyOnItemAdded('EndPoint', response, 'Name')) {
					if ($scope.onEndPointAdded != undefined) {
						var insertedItems = response.InsertedObject;
						if (insertedItems != undefined)
							for (var i = 0; i < insertedItems.length; i++) {
								$scope.onEndPointAdded(insertedItems[i]);
							}
					}
					$scope.modalContext.closeModal();
				}
		...
		function update() {
			var endPoint = buildEndPointObjFromScopeForUpdate();
		...
		function buildEndPointObjFromScopeForAdd() {
			var EndPointsToAdd = [];
			if ($scope.scopeModel.endpointtype == NP_IVSwitch_EndPointEnum.ACL.value) {
					...
					for (var i = 0; i < allHosts.length; i++) {
						var endPointToAdd = {
							Entity:
							{
								...
								Host: $scope.scopeModel.subnet != undefined ? allHosts[i] + "/" + $scope.scopeModel.subnet : allHosts[i],
						...
						EndPointsToAdd.push(endPointToAdd);
					}
				}
			}
			else {
				var allSipLogPass = $scope.scopeModel.sipLoginPasswords;

				if (allSipLogPass != null) {
					for (var i = 0; i < allSipLogPass.length; i++) {
						var endPointToAdd1 = {
							Entity:
							{
								EndPointId: endPointEntity != undefined ? endPointEntity.EndPointId : undefined,
								...
								DomainId: $scope.scopeModel.domaineid,
								TechPrefix: $scope.scopeModel.techprefix,
								SipLogin: allSipLogPass[i].SIPLogin,
								SipPassword: allSipLogPass[i].SIPPassword,
								EndPointType: $scope.scopeModel.endpointtype,
								RouteTableBasedRule: $scope.scopeModel.routeTableBasedRule
							},
							CarrierAccountId: carrierAccountId
						};
						EndPointsToAdd.push(endPointToAdd1);
					}
				}
			}

			return { EndPointsToAdd: EndPointsToAdd, EndPointType: $scope.scopeModel.endpointtype };


		}	  

		function buildEndPointObjFromScopeForUpdate() {
			return {
				Entity:
				{
