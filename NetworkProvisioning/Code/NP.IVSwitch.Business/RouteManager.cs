...
namespace NP.IVSwitch.Business
{
	public class RouteManager : IRouteManager
	{
		...
		public InsertOperationOutput<List<RouteDetail>> AddRoute(MultipleRoutesToAdd routeItems)
		{
			InsertOperationOutput<List<RouteDetail>> insertOperationOutput = new InsertOperationOutput<List<RouteDetail>>
			{
				Result = InsertOperationResult.Failed,
				InsertedObject = new List<RouteDetail>()
			};
			int routeId;
			string mssg;
			...
			{
				foreach (var routeItem in routeItems.RoutesToAdd)
				{
					var recurringHost = routeItems.RoutesToAdd.Count(item => item.Entity.Host.Equals(routeItem.Entity.Host));
					if (recurringHost > 1)
					{
						insertOperationOutput.Message = "Recurring same Host";
						insertOperationOutput.ShowExactMessage = true;
					}
				}
				foreach (var routeItem in routeItems.RoutesToAdd)
				{
					if (Insert(routeItem, out routeId, out mssg))
					{
						...
						insertOperationOutput.InsertedObject.Add(RouteDetailMapper(GetRoute(routeId)));
					}
					else
					{
		...
		public RouteEntityInfo RouteEntityInfoMapper(Route route)
		{
			...
			RouteEntityInfo routeInfo = new RouteEntityInfo()
			{
				...
				Description = string.Format("{0}  ({1})", GetRouteDescription(route), Utilities.GetEnumDescription(route.CurrentState)),
